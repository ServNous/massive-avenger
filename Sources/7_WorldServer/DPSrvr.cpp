		...
		if( sChat[0] == '/'  && ParsingCommand( strChat, (CMover*)pUser ) )
{
		...
		strPatch.Format( "..\\DSystem\\Logs\\chat-MJ\\%s", pUser->GetName() );
...
//LOG chat
		...
		strPatch.Format( "..\\DSystem\\Logs\\chat\\%s", pUser->GetName() );
		DSystemLog( strPacket, strPatch, 0 );
//LOG chat
			return;
}
		if( pUser->IsMode( TALK_MODE ) )
{
		if( pUser->m_dwAuthorization >= AUTH_GAMEMASTER )		// ÀÏ¹ÝÀ¯Àú°¡ ¾Æ´Ï¸é ·Î±×³²±è ¸ðµç ·Î±×³²±è
		{
				CString strPacket;
		CString strPatch;
		strPacket.Format( "[DSystem] Chat-MJ -> %s", sChat);
...
//LOG chat
		CString strPacket;
		CString strPatch;
		strPacket.Format( "[DSystem] Chat -> %s", sChat);
...
void CDPSrvr::OnRevivalLodestar( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	{
		//CColosseumMatch::KickUser(idMember);
	}
#endif
		if( pWorld->GetID() == WI_WORLD_GUILDWAR )
		{
...
void CDPSrvr::OnGetItemGuildBank( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		else if (mode == 1) // ¾ÆÀÌÅÛÀ» ±æµåÃ¢°í¿¡¼­ »©³¾¶§
		{
			...
			{
				if( nId < 1 || nId > pGuild->m_GuildBank.GetMax() )
					return;
...
void CDPSrvr::OnModifyMode( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		if( pUser->m_Snapshot.dpidUser != dpid )
		{
			...
			CString strPacket;
			strPacket.Format( "[DSystem] Utilisateur: %s -> Changement de mode : %s ", pUser->GetName(), pUser->m_dwMode);
...
void CDPSrvr::OnTradelastConfrim( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
		...
		IsValidObj( pTrader ) && pTrader->GetWorld() == pWorld && pTrader->m_vtInfo.GetOther() == pUser )
	{
		...
		case TRADE_STEP_OK:
			...
			pUser->AddText( "#b[LTD]#nb Source par dave devils." );
			#endif
...
void CDPSrvr::OnBuyItemCart( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) && pUser->m_vtInfo.GetOther() )
	{
		...
		{
			if( pCartItem.cTab[i] >= MAX_VENDOR_INVENTORY_TAB || pCartItem.cTab[i] < 0 || pCartItem.dwNum[i] < 1 )
				continue;
