...
void CDPCoreClient::OnSetPartyMode( CAr & ar, DPID, DPID, OBJID )
{
	...
	if( pParty )
	{
		...
		// 090917 mirchang - ¸ðµå°¡ ÆÄ½ºÅ³Ç® ÀÌ°í bOnOfff°¡ FALSE ÀÏ¶© ±Ø´Ü¿øÁß ÆÄ½ºÅ³Ç® ¾ÆÀÌÅÛ »ç¿ëÁßÀÎÁö Ã¼Å©ÇÏ¿© »ç¿ëÁßÀÎ ±Ø´Ü¿øÀÌ ÀÖÀ»¶© ´Ù½Ã ¸ðµå ¼³Á¤
...
void CDPCoreClient::OnGuildContributionACK( CAr & ar, DPID, DPID, OBJID )
{
	...
	if( pGuild )
	{
		if( uServerID != ::g_uKey )		// ÀÌ ¸Þ¼¼ÁöÀÇ ¿øº»¼­¹ö°¡ ¾Æ´Ñ °æ¿ì¸¸ update
...
void CDPCoreClient::OnGuildPenya( CAr & ar, DPID, DPID, OBJID )
{
    ...
    ar >> uGuildId;
    ar >> dwType >> dwPenya;
if( dwType < 0 || dwType >= MAX_GM_LEVEL ) // Limit of dwType here - Limite le dwType ici
          return;
    CGuild* pGuild    = g_GuildMng.GetGuild( uGuildId );
    if( pGuild )
    {
        ...
        for( map<u_long, CGuildMember*>::iterator i = pGuild->m_mapPMember.begin(); i != pGuild->m_mapPMember.end(); ++i )
        {
            ...
            pUser    = (CUser*)prj.GetUserByID( pMember->m_idPlayer );
            if( IsValidObj(pUser) )
                pUser->AddSetGuildPenya( dwType, dwPenya ); // Here the overflow - ici le dépassement dans dwType
...
// ÄÚ¾î¼­¹ö¿¡ (Çö»ó±Ý¾ò±â & Çö»ó±Ý clear) ¿äÃ»  
#ifdef __LAYER_1015
...
{
	DWORD dwSerial;
	ar >> dwSerial;		// CACHE¿¡¼­ »ý¼ºµÈ serialÇÑ °ª 
