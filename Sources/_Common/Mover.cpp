...
void CMover::Init()
{
	...
	m_nCount			= xRandom( 128 );		// ¸®½ºÆù µÇ´Â ¸÷¸¶´Ù ½ÃÀÛÄ«¿îÅÍ¸¦ ¾à°£¾¿ Æ²¸®°Ô ÇØ¼­ Á»´õ ´Ù¾çÇÑ ¹ÝÀÀÀÌ ³ª¿À°Ô Çß´Ù.
	m_bPlayer			= FALSE;
	m_dwTypeFlag		= 0;
	m_pRide				= NULL;  
	m_pAIInterface		= NULL;
	...
	m_idLastHitMover	= NULL_ID;			// this°¡ ¸¶Áö¸·À¸·Î ÃÆ´ø ¹«¹ö¾ÆÀÌµð
	m_nFame				= 0;				// ¸í¼º
	m_nDuel				= 0;				// µà¾óÁßÀÎ°¡.
...
#if __VER >= 13 // __HONORABLE_TITLE			// ´ÞÀÎ
	m_nHonor = -1;					// ´ÞÀÎ¼±ÅÃ 
	m_dwHonorCheckTime = 0;			//´ÞÀÎ ½Ã°£Ã¼Å©
...
// AI ¸Å½ÃÁö º¸³¿ : °´Ã¼ÀÇ Çàµ¿ ÆÐÅÏ, ÇöÀç »óÅÂ¸¦ ¸Å½ÃÁö·Î Á¦¾îÇÔ 
...
void CMover::SubSMMode()
{
	...
	for( int i = 0 ; i < SM_MAX ; ++i )
	{
		if( m_dwSMTime[i] > 0 )
		{
			if( g_AddSMMode.bSMModetime[i] )	// Àý´ë½Ã°£
			{
...
{
	if( g_AddSMMode.bSMModetime[nType] )	// Àý´ë½Ã°£
	{
...
void CMover::ProcessQuest()
{
	...
	if( lpCharacter )
	{
		...
		m_nQuestEmoticonIndex = -1;
#if __VER >= 15 // __IMPROVE_QUEST_INTERFACE
		// NPC Äù½ºÆ® ÀÌ¸ðÆ¼ÄÜ ¿ì¼±¼øÀ§ º¯°æ - ¿Ï·á > ½Å±Ô > ÁøÇà > ±Ù·¹º§
		for( int i = 0; i < lpCharacter->m_awSrcQuest.GetSize() ; i++ )
		{
			...
			if( lpQuest == NULL && pMover->IsCompleteQuest( nQuest ) == FALSE )
			{
				...
				if( __IsBeginQuestCondition( pMover, nQuest ) && ( nItem == 0 || pMover->GetItemNum( nItem ) ) )
					m_nQuestEmoticonIndex = 1;
				// »õ·Î¿î Äù½ºÆ®°¡ ¾ø°í ±Ù·¹º§¿¡ ¹ÞÀ» ¼ö ÀÖ´Â Äù½ºÆ®°¡ Á¸ÀçÇÑ´Ù¸é..
...
void CMover::ProcessAbnormalState()
{
	...
	if( nHeal > 0 )
	{
		...
		if( m_nHealCnt == 0 )
		{
			if( IsLive() )
			{
				m_nHealCnt	= (short)( PROCESS_COUNT );
				nHeal = nHeal + GetHitPoint();
...
// ¸ó½ºÅÍÀÇ °æ¿ì Á¤±âÀûÀ¸·Î idAttacker, m_idTargeter¸¦ °Ë»çÇØ¼­ ¾ø¾îÁø³ðÀÌ¸é »èÁ¦ÇÔ.
...
void CMover::InitLevelDave( int nJob, LONG nLevel, BOOL bGamma )
{
#ifdef __WORLDSERVER
	// ¿î¿µÀÚ ¸í·ÉÀ¸·Î ·¹º§¾÷ ÇÏ´Â°÷ÀÓ
	MoverProp* pProp = GetProp();
	if( pProp )
	{
		...
		if(IsMaster())
		{
			...
			if( nLevel > 59 && m_nLevel < 72 )
				dwTmpSkLevel = 1;
			else if( nLevel > 71 && m_nLevel < 84 )
				dwTmpSkLevel = 2;
			else if( nLevel > 83 && m_nLevel < 96 )
				dwTmpSkLevel = 3;
			else if( nLevel > 95 && m_nLevel < 108 )
				dwTmpSkLevel = 4;
			else if( nLevel > 107 && m_nLevel < 120 )
				dwTmpSkLevel = 5;
			for( int i = 0; i < MAX_SKILL_JOB; i++ ) 
			{				
				LPSKILL lpSkill = &(m_aJobSkill[i]);
		...
		if( m_nJob == JOB_MERCENARY )
			...
			nLevel = 130;
		else if( nJob ==  JOB_KNIGHT_MASTER || nJob ==  JOB_JESTER_MASTER || nJob ==  JOB_RANGER_MASTER || nJob ==  JOB_RINGMASTER_MASTER || nJob ==  JOB_BILLPOSTER_MASTER || nJob ==  JOB_ELEMENTOR_MASTER )
			nLevel = 60;
		else if( m_nJob ==  JOB_BLADE_MASTER )
			nLevel = 60;
		else if( m_nJob ==  JOB_PSYCHIKEEPER_MASTER )
			nLevel = 60;
		else if( nJob ==  JOB_KNIGHT_HERO || m_nJob ==  JOB_BLADE_HERO  || m_nJob ==  JOB_PSYCHIKEEPER_HERO || nJob ==  JOB_JESTER_HERO || nJob ==  JOB_RANGER_HERO || nJob ==  JOB_RINGMASTER_HERO || nJob ==  JOB_BILLPOSTER_HERO || nJob ==  JOB_ELEMENTOR_HERO )
			nLevel = 121;
		else
			nLevel = 15;
		nLevel = m_nLevel;
...
int	CMover::DoDropItemRandom( BOOL bExcludeEquip, CMover* pAttacker, BOOL bOnlyEquip )
{
	...
	while( nRealMax )	// °Ë»ö´ë»ó ¾ÆÀÌÅÛÀÌ ´õÀÌ»ó ¾øÀ»°æ¿ì ³¡.
	{
		...
		pItemElem = pElemBuff[ nIdx ];		// ¾ÆÀÌÅÛ ¸®½ºÆ®¿¡¼­ ·£´ýÀ¸·Î ÇÏ³ª¸¦ »Ì¾Æ¿È
...
#endif // __VER >= 8 // __S8_PK			
		if( m_Inventory.IsEquip( pItemElem->m_dwObjId ) )	// ÀåÂøÇÏ°í ÀÖÀ¸¸é ¹þ±è 			
		{
		...
		// drop¿¡ ½ÇÆÐÇÑ ¾ÆÀÌÅÛÀÌ¶óµµ ¸®½ºÆ®¿¡¼­ »©ÁÖÀÚ.
...
BOOL CMover::IsUseItemReadyTime( ItemProp* pItemProp, OBJID dwObjItemId )
{
	...
	if( pItemProp->dwParts == PARTS_RIDE )
	{
		...
		if( !pWorld->m_bFly || nAttr == HATTR_NOFLY ) // ºñÇà±ÝÁö±¸¿ªÀÔ´Ï´Ù.
		{
		...
		pItemProp->dwSkillReadyType != 0xffffffff )
	{
		...
		else if( IsStateMode( STATE_BASEMOTION_MODE ) )	// ÀÌ¹Ì ½ÃÀüÁßÀÌ¸é ¸Þ¼¼Áö Ã³¸®
		{
			PrintString( this, TID_PK_BLINK_LIMIT );	// ½ÃÀüÁß¿¡´Â »ç¿ëÇÒ¼ö ¾ø½À´Ï´Ù
			return FALSE;		
		}
		else if( m_pActMover->IsState( OBJSTA_STAND ) == FALSE )	// ¼­ÀÖ´Â »óÅÂ°¡ ¾Æ´Ï¸é ¸Þ¼¼Áö Ã³¸®
		{
...
ItemProp* CMover::GetActiveHandItemProp( int nParts )
{
	if( IsPlayer() )
	{
		...
		else	// Å¬¶óÀÌ¾ðÆ®¿¡¼­´Â Å¸ Ä³¸¯ÅÍ´Â m_adwEquipment¿¡ dwObjId°¡ ¾Æ´Ñ dwItemId¸¦ °¡Áö°í ÀÖ´Ù.
		{
	...
	// ±× ¾ÆÀÌÅÛÀº ÁÖ·Î ¹«±â³ª, ½ºÅ³À» ÀÇ¹ÌÇÒ °ÍÀÌ¸ç, ÁÖ·Î °ø°Ý¿ëÀ¸·Î »ç¿ëµÉ °ÍÀÌ´Ù.
...
// ÇÃ·¹ÀÌ¾îµéÀÌ ¶Û¶§ »ç¿îµå¼Ò¸®¸¦ ³½´Ù...ÄÝ¶ó¿¡¼­ »ç¿îµåÁöÁ¤À¸·Î ÇÃ·¹ÀÌÇÏ´Â°ÍÀÌ ¾Æ´Ô.
...
void CMover::ProcessETC()
{
	...
	// ±æµåÄÄ¹î ¸ÊÀÌ¶ó¸é...ºÎÇÏ¸¦ ÁÙÀÌ±â À§ÇØ »óÅÂ¿¡ µû¶ó ·£´õ¸µÀ» ±ÝÁöÇÑ´Ù.
...
void CMover::Process()
{
	...
	{
		PROFILE( sw, r3 );
		ProcessAI();							// 1È¸ ÀÌµ¿ÇÏ¸é ProcessAI°¡ ¹Ýµå½Ã È£ÃâµÇ¾î¾ß ÇÑ´Ù.
	}
	...
	{
		PROFILE( sw, r4 );
		m_pActMover->ProcessPreAction();		// ÇöÀç Çàµ¿ »óÅÂ¿¡ µû¶ó ÀûÀýÇÑ	 ¸ð¼ÇÀ» Ã³¸®ÇÔ
		ProcessAniSpeed();						// °ø¼Ó ¾Ö´Ï¸ÞÀÌ¼Ç ¼Óµµ Á¶Àý.
	}
...
//					FLOAT fMaxHeight	= pModel->GetMaxHeight();
				...
				pModel->FrameMove( &vPos, m_fAniSpeed * fFactor );		// ¾Ö´Ï¸ÞÀÌ¼Ç ÇÁ·¹ÀÓ Áõ°¡
#else	// __AI_0509
				pModel->FrameMove( &vPos, m_fAniSpeed );		// ¾Ö´Ï¸ÞÀÌ¼Ç ÇÁ·¹ÀÓ Áõ°¡
#endif	// __AI_0509
			#ifdef __CLIENT			
				ProcessDustSFX();
			#endif // CLIENT
		...
		if( m_nAtkCnt > 0 )		// °ø°ÝÇàÀ§¸¦ ÇÏ°í³­ ÀÌÈÄ´Â Ä«¿îÆ®°¡ °è¼Ó Áõ°¡ ÇÑ´Ù.
			++m_nAtkCnt;
	}
	/*
#ifdef __CLIENT
	...
	{
		PROFILE( sw, r6 );
		bCollision = m_pActMover->ProcessCollision( &vPos );	// ÀÌµ¿¼ººÐ ´õÇÏ±â & Ãæµ¹Ã³¸®.
	}
	...
	++m_nCount;		// ¹«¹ö°¡ ¹ü¿ëÀ¸·Î ¾²´Â ¼øÂ÷Àû Ä«¿îÅÍ. »ý¼ºÀÚ ¿Ü¿£ 0À¸·Î ÃÊ±âÈ­ ÇÏÁö ¸»°Í.
...
void CMover::AngelMoveProcess()
{
	if( m_pAngelFlag && m_pAngel != NULL )
	{
		...
		if(fDistSq > 100.0f * 100.0f) //Teleportµî °Å¸®°¡ ¸Ö¾îÁú °æ¿ì ´Ù½Ã »ý¼ºÇÏµµ·Ï ÇÔ.
		{
			if(m_pAngel)
				SAFE_DELETE(m_pAngel);
			m_pAngel = NULL;
...
//	nAbilityOption = 4;
    ...
    if(m_nStr > 400) //Si l'STR est plus de 400
    {
#ifdef __GLOW_NEW_STAT
    dwSfx = XI_DAVE_ITEM_STR; break;
#else
 	dwSfx = XI_GEN_ITEM_SETITEM10; break;
#endif
    }
    else if(m_nSta > 400) //Si l'STA est plus de 400
    {
#ifdef __GLOW_NEW_STAT
    dwSfx = XI_DAVE_ITEM_STA; break;
#else
 	dwSfx = XI_GEN_ITEM_SETITEM10; break;
#endif
    }
    else if(m_nDex > 400) //Si le DEX est plus de 400
    {
#ifdef __GLOW_NEW_STAT
    dwSfx = XI_DAVE_ITEM_DEX; break;
#else
 	dwSfx = XI_GEN_ITEM_SETITEM10; break;
#endif
    }
    else if(m_nInt > 400) //Si l'INT est plus de 400
    {
#ifdef __GLOW_NEW_STAT
    dwSfx = XI_DAVE_ITEM_INT; break;
#else
 	dwSfx = XI_GEN_ITEM_SETITEM10; break;
#endif
    }
    else if(m_nInt > 200 && m_nDex > 200 && m_nStr > 200 && m_nSta > 200) //si tout les stat
    { 
#ifdef __GLOW_NEW_STAT
    dwSfx = XI_DAVE_ITEM_GOD; break; //On met l'aura ultime
#else
 	dwSfx = XI_GEN_ITEM_SETITEM10; break;
#endif
    }
...
/*	if (pItemBase->GetProp()->dwItemKind2 == IK2_CLOTHETC || pItemBase->GetProp()->dwItemKind2 == IK2_CLOTH)
	{
	return m_Inventory2.GetAtId (dwId);
	}
	...
	{
	return m_Inventory3.GetAtId (dwId);
	}
	else if(pItemBase->GetProp()->dwItemKind2 == IK2_GEM )
	{
	return m_Inventory4.GetAtId (dwId);
	}
	else
	{
...
void CMover::RemoveItemId( DWORD dwId )
{
	if( m_Inventory.IsEquip( dwId ) )
		DoEquip( (CItemElem*)m_Inventory.GetAtId( dwId ), FALSE ); // En cas que il en soit equiper
#ifdef __TAB_INVENTORY
/*	if (pItemBase->GetProp()->dwItemKind2 == IK2_CLOTHETC || pItemBase->GetProp()->dwItemKind2 == IK2_CLOTH)
	{
	m_Inventory2.RemoveAtId( dwId );
	}
	else if (pItemBase->GetProp()->dwItemKind3 == IK3_PET || pItemBase->GetProp()->dwItemKind3 == IK3_EGG || pItemBase->GetProp()->dwItemKind3 == IK3_FEED)
	{
	m_Inventory3.RemoveAtId( dwId );
	}
	else if(pItemBase->GetProp()->dwItemKind2 == IK2_GEM )
	{
	m_Inventory4.RemoveAtId( dwId );
	}
	else
	{
...
BOOL CMover::SetMotion( DWORD dwMotion, int nLoop, DWORD dwOption )
{
	...
	if( m_pActMover->IsStateFlag( OBJSTAF_COMBAT ) && (dwOption & MOP_FIXED) == 0 )
	{
		...
		// ÇÃ·¹ÀÌ¾î´Â ¹«±â Á¾·ù¿¡ µû¶ó¼­ ¸ð¼ÇÀ» °áÁ¤ÇÏÁö¸¸, ¸÷Àº virtual ¾ÆÀÌÅÛ¿¡¼­ ¸ð¼ÇÀ» °áÁ¤ÇÑ´Ù.
		int nIndex = GetIndex();
...
#if __VER >= 10 // __LEGEND	//	9Â÷ Àü½Â½Ã½ºÅÛ	Neuz, World, Trans
#if	__VER >= 16 //__3RD_LEGEND16 >= 16 //__3RD_LEGEND16
	...
	if( m_dwMotion == dwOrigMotion && m_dwMotionOption == dwOption )	// °°Àº ¸ð¼ÇÀ» ÇÏ¶ó°í Çß´Âµ¥...
	{
	...
	// ´« »ç¶÷ ¾Ö´Ï¸ÞÀÌ¼Ç ¾ÈÇÏ´Â Mover¿¡¼­ »ý±â´Â ¹®Á¦ ¶§¹®¿¡ ¿¹¿ÜÃ³¸®.
...
int CMover::DoDie( CCtrl *pAttackCtrl, DWORD dwMsg )
{
	...
	if( IsPlayer() && m_nDead )
	{
		Error( "CMover::DoDie : %s , Mort et il tape toujous", GetName() );
		return 0;
	}
	...
	if( IsPlayer() )
	{
		...
		{
			if( pLastHit->m_idAttacker == GetId() )	// pLastHitÀÇ °ø°ÝÀÚ°¡ ³ª·Î ¼³Á¤µÇ¾î ÀÖÀ»¶§¸¸ Ç®¾îÁÜ,.
...
#if __VER >= 9	// __PET_0410
			// Ä³¸¯ÅÍ°£ÀÇ ÀüÅõ·Î ÀÎÇÑ ¸ðµç Ä³¸¯ÅÍÀÇ »ç¸Á ½Ã ÆêÀº »ç¸ÁÇÏÁö ¾Ê´Â´Ù.	// 0723
...
//		g_DPCoreClient.SendMoverDeath( this, pAttacker, dwMsg );
	...
	{
		pAttacker->SendAIMsg( AIMSG_INIT_TARGETCLEAR );		// °ø°ÝÀÚ´Â Å¸°ÙÀ» Å¬¸®¾îÇÏ°í ´ë±â¸ðµå·Î µ¹¾Æ°¨.
	}
...
void	CMover::DoPVPEnd( CCtrl *pAttackCtrl, bool bWinner , DWORD dwMsg )
{
	...
	if( pAttackCtrl && pAttackCtrl->GetType() == OT_MOVER )		//  ¹«¹ö¶ó¸é ¹«¹ö Æ÷ÀÎÅÍ ¸¸µé¾î µÐ´Ù.
		pAttacker = (CMover *)pAttackCtrl;
	else
		return;
...
//	pAttacker->m_nNumKill++;				// remis mais commenté par ToX82
//	((CUser*)this)->AddDefinedText( TID_GAME_PKDEAD, "\"%s\"", pAttacker->m_szName );	// ??¿¡°Ô Á×À½À» ´çÇÏ¿´½À´Ï´Ù.
...
#ifdef __LAYER_1015
		g_DPCoreClient.SendWCWantedReward( m_idPlayer, pAttacker->m_idPlayer, szFormat, pWorld->GetID(), GetPos(),this->GetLayer() );
#else	// __LAYER_1015
	...
	else							// ÇÎÅ©»óÅÂ, Èù»ö»óÅÂ
	{
		...
		/// Ä«¿À¼öÄ¡, ¼ºÇâÀ» ¿Ã·ÁÁÜ
	if( bAdd && nReflect == 0 
		&& pAttacker != this ) // chipi_081119 Áö¼ÓÇü µ¥¹ÌÁö ½ºÅ³ ¹Þ°í ÀçÁ¢ÇÏ¸é ÀÚ»ìµÇ¾î Ä«¿ÀµÇ´Â ¹®Á¦ ¼öÁ¤...
	{
		if( IsPlayer() && pAttacker->IsPlayer() )
		{
...
if(!pAttacker->IsPlayer())
	...
	m_idMurderer = pAttacker->m_idPlayer;	// ³¯ ¸¶Áö¸·À¸·Î Á×ÀÎ³ðÀÇ ÇÃ·¹ÀÌ¾î¾ÆÀÌµð¸¦ ÀúÀåÇØµÒ.
...
PVP_MODE CMover::GetPVPCase( CMover *pAttacker )
{
	...
	if( bPKEnable == FALSE || 
		pAttacker == NULL  || 
		pAttacker->IsNPC() || // Á×ÀÎ³ðÀÌ NPC¸é °Á ¸®ÅÏ.
		IsNPC() )			  // Á×Àº³ðÀÌ NPC¸é °Á¸®ÅÏ
		return PVP_MODE_NONE;

	// ÀÌ°Ç »ç¶÷´ë »ç¶÷ÀÇ ½Î¿òÀÌ´Ù!
	if( m_idWar && pAttacker->m_idWar == m_idWar )	// ³ª¶û °°Àº ÀüÀï¿¡ Âü°¡ÁßÀÎ ³ðÀÎ°¡.
	{
...
// this°¡ Á×Àº¹«¹ö´Ù.
// this¸¦ Áß½ÉÀ¸·Î ¹Ý°æ fRange ÀÌ³»¿¡ ÀÖ´Â »ç¶÷µé¿¡°Ô °æÇèÄ¡¸¦ ºÐ¹èÇÑ´Ù.
...
#define MAX_AROUND_USER		512
	...
	FOR_VISIBILITYRANGE( this )
	{
		...
		if( fDistSq <= fRange )				// ¹Ý°æ fRange ¹ÌÅÍ ÀÌ³»¿¡ ÀÖ´Â »ç¶÷Àº
		{
			*ptr++ = pUser;
			nMaxList ++;
		}
	...
	EXPINTEGER nExp		= GetProp()->nExpValue / (EXPINTEGER)nMaxList;	// 1ÀÎ´ç ¾ó¸¶¾¿ ¹èºÐµÇ¾ß ÇÏ´Â°¡.
...
#ifdef __PVP_EXP //experiance pour du pvp ? :o merci yanikmama
			if( pDead->IsPlayer() )
			{
				if( GetLevel() >= MAX_LEGEND_LEVEL && ( GetLevel() - 19 ) >= pDead->GetLevel() && ( GetLevel() + 19 ) <= pDead->GetLevel() )
				{
					fExpValue	= static_cast<EXPFLOAT>( 62582890 );
					fFxpValue	= (float)62582890.0f;
				}
			}
			else if ( GetLevel() < MAX_LEGEND_LEVEL )
			{
...
void CMover::AddExperienceKillMember( CMover *pDead, EXPFLOAT fExpValue, MoverProp* pMoverProp, float fFxpValue )
{
	...
	for( DWORD j = 0; j < adwEnemy.size(); j++ )
	{
		...
		if( IsValidObj( pEnemy ) && pDead->IsValidArea( pEnemy, 64.0f ) && pEnemy->IsPlayer() )		// ÇÃ·¹ÀÌ¾î, ¹üÀ§ °Ë»ç
		{
			...
			if( dwHitPointParty )		// ±Ø´Ü °æÇèÄ¡ ºÐ¹è
			{	
				int nTotalLevel		= 0;
				int nMaxLevel10	= 0;
				int nMaxLevel	= 0;
...
#ifdef __BOITE_XP   // 8Â÷ ½ºÅ³°æÇèÄ¡´Ù¿îº¯°æ
		// Á×À½ ÀÌÈÄ »óÅÂ¶ó¸é °æÇèÄ¡¸¦ 150%ÁØ´Ù.
		if( IsAfterDeath() )
			fExpValue	*= static_cast<EXPFLOAT>( 1.5f );
#endif	// boite xp 

#ifdef __ECHO_BERSERK
...
void CMover::AddExperienceParty( CMover *pDead, EXPFLOAT fExpValue, MoverProp* pMoverProp, float fFxpValue, CParty* pParty, CUser* apMember[], int* nTotalLevel, int* nMaxLevel10, int* nMaxLevel, int* nMemberSize )
{
	...
	{
		switch( pParty->m_nTroupsShareExp ) // °æÇèÄ¡ ºÐ¹è ¹æ½Ä
		{
		case 1 :	// ±â¿©ºÐ¹è
			{
...
{
	// È¹µæÇÑ °æÇèÄ¡ = ÀÚ½ÅÀÇ Å¸°Ý¿¡ ÀÇÇÑ °æÇèÄ¡ + º¸³Ê½º °æÇèÄ¡ + ¿É¼Ç (¸ó½ºÅÍ°æÇèÄ¡ * ÆÄÆ¼¿øÁß ¶§¸°³ðµé / 100 )
	...
	for( i = 0 ; i < nMemberSize ; i++ )
	{
		...
		if( nHit )
		{
			...
			if( nMaxLevel10 < apMember[i]->GetLevel() )
			{
				...
				{
					if( nHitPercent <= 10 )
 						nMemberExp	*= static_cast<EXPINTEGER>(1.2f);
					else
						nMemberExp	+= static_cast<EXPINTEGER>(( nMemberExp * 0.2f ) - ( ( nMemberExp * 0.2f ) * nHitPercent / CRITICAL_BERSERK_HP ));
				}
#endif // __S_9_ADD
			...
			if( nMaxLevel10 < apMember[i]->GetLevel() )
			{
				...
				if( nHitPercent < CRITICAL_BERSERK_HP )
				{
					if( nHitPercent <= 10 )
						...
						nMemberExp	+= static_cast<EXPINTEGER>(( nMemberExp * .2f ) - ( ( nMemberExp * .2f ) * nHitPercent / CRITICAL_BERSERK_HP ));
				}
#endif // __S_9_ADD
				TRACE("GetExp(Contribution) : %s -> %I64d\n ", apMember[i]->GetName(), static_cast<EXPINTEGER>( nMemberExp ) );
				AddPartyMemberExperience( apMember[i], nMemberExp, 0 );
...
//	int	  nMemberFxp;
	...
	for( i = 0 ; i < nMemberSize ; i++ )
	{
		if( nMaxLevel10 < apMember[i]->GetLevel() )
		{
			//[ ¸ó½ºÅÍÀÇ °æÇèÄ¡ + {(¸ó½ºÅÍ °æÇèÄ¡ * 0.15) * (±Ø´Ü ÀÎ¿ø - 1)} * (ÀÚ½ÅÀÇ ·¹º§ÀÇ Á¦°ö/¸ðµç ÆÄÆ¼¿ø ·¹º§ Á¦°öÀÇ ÇÕ) ]
...
BOOL CMover::IsAttackAbleNPC( CMover* pNPC )
{
	...
	else
	{
		...
		if( pMoverProp->bKillable == 0 )	
			return FALSE;		// °ø°ÝÀÌºÒ°¡´ÉÇÑ NPC´Â ¹«Á¶°Ç °ø°Ý¸øÇÔ.
		
		// ÆòÈ­ÀûÀÌ¸é ¸ø¶§¸².
		if( pNPC->IsPeaceful() == TRUE )	// ÆòÈ­ÀûÀÎ³ÑÀº °ø°Ý ¾ÈµÊ
		{
			bAble = FALSE;
		}
		else
		{
			...
			{
				if( pMoverProp->nChaotic < 0 && IsChaotic() == TRUE )			// ¼ºÇâÀÌ ¼­·Î °°À¸¸é °ø°Ý¸øÇÔ.
					bAble = FALSE;
				else if( pMoverProp->nChaotic > 0 && IsChaotic() == FALSE )		// ¼ºÇâÀÌ ¼­·Î °°À¸¸é °ø°Ý¸øÇÔ.
					bAble = FALSE;
			}

			if( m_pActMover->IsFly() && pMoverProp->dwFlying == 0 )	// °ø°ÝÀÚ°¡ ºñÇàÁßÀÌ°í Å¸°ÙÀÌ ºñÇà¸÷ÀÌ ¾Æ´Ï¸é
				bAble = FALSE;
			else if( m_pActMover->IsFly() == FALSE && pMoverProp->dwFlying == 1 )	// °ø°ÝÀÚ Áö»ó / Å¸°Ù ºñÇà¸÷
				bAble = FALSE;
		}
	}
...
#ifdef __CLIENT
	if( bAble == TRUE )
		if( GetAsyncKeyState(VK_CONTROL) & 0x8000 )	// NPC¿¡°Ô °ø°ÝÅ°¸¦ ´©¸£°í ÀÖÀ»¶§ Å¬¸¯ÇÏ¸é ÀÚµ¿°ø°Ý.
...
BOOL CMover::IsPKInspection( CMover* pOther )
{
	...
	{
		if(GetWorld() && GetWorld()->GetID() == WI_WORLD_GUILDWAR && IsGuildCombatTarget( pOther ) )	// ±æµå´ëÀü ÁßÀÎ À¯Àú¿¡°Ô´Â ¸Þ¼¼Áö°¡ ¾È³ª¿À°Ô ÇÏÀÚ
...
// pTargetÀº ¹Ýµå½Ã IsValid Ã¼Å©¸¦ ÇÏ°í ³Ñ¾î¿Í¾ß ÇÑ´Ù.
#ifdef __WORLDSERVER
int CMover::IsSteal( CMover *pTarget )
{
	if( IsNPC() || ::GetLanguage() == LANG_JAP )	
		return 0;		// °ø°ÝÀÚ°¡ NPC¸é ½ºÆ¿ Ã¼Å©´ë»ó ¾Æ´Ô.
...
#ifndef __PK_V6
	if( pTarget->IsNPC() )	// °ø°ÝÀÚ:ÇÃ·¹ÀÌ¾î, ¸Â´ÂÀÚ:NPC 
#else // __VER >= 8 // __S8_PK
	if( pTarget->IsNPC() || (pTarget->IsPlayer() && pTarget->IsChaotic() == FALSE) )	// °ø°ÝÀÚ:ÇÃ·¹ÀÌ¾î, ¸Â´ÂÀÚ:NPC È¤Àº ÂøÇÑ»ç¶÷. (½½·ÎÅÍ´Â ´Ù±¸¸®´çÇØµµ µÈ´Ù).
#endif // __VER >= 8 // __S8_PK
	{
		...
		{
			// °ø°ÝÀÚ°¡ ÆÄÆ¼°¡ ÀÖ°í pTarget¸¦ ¶§·È´ø »ç¶÷ÀÌ °°Àº ÆÄÆ¼¸é ½ºÆ¿ÀÌ ¾Æ´Ï´Ù.
		...
		if( bStealCheck && 
			pTarget->m_idTargeter != NULL_ID && pTarget->m_idTargeter != GetId()) // pTarget¸¦ ÂòÇÑ³ðÀÌ ÀÖ°í ±×°Ô ÇöÀç°ø°ÝÀÚ°¡ ¾Æ´Ï¶ó¸é ½ºÆ¿.
		{
...
void CMover::GetDieDecExpRate( FLOAT& fDecExp, DWORD dwDestParam, int nSlaughter )
{
	...
	// Ä«¸£¸¶ µî±ÞÀº ±âÁ¸ÀÇ »ç¸Á½Ã °æÇèÄ¡ ÇÏ¶ô¿¡µµ ¿µÇâÀ» ¹ÌÄ£´Ù.
...
BOOL CMover::DropItem( CMover* pAttacker )
{
	...
	if( pAttacker->IsPlayer() && IsNPC() )
	{
		...
		for( int k = 0 ; k < nloop ; k++ )
		{
			...
			BOOL bAdjDropRate = TRUE;	// ÀÏ¹ÝÀûÀ¸·Ð ·¹º§Â÷¿¡ ÀÇÇÑ µå¶ø·üÀúÇÏ°¡ ÀÌ·ïÁø´Ù.
// Le gap

			if( GetIndex() == MI_CLOCKWORK1 )	//·¹º§Â÷¿¡ÀÇÇÑ µå¶ø·üÀúÇÏ°¡ ¾ø´Ù.
				bAdjDropRate = FALSE;
#ifdef __LECTURE_V19
			if( lpMoverProp->m_nGapOff == 1 )
				bAdjDropRate = FALSE;
#endif
			...
			if( xRandom( 100 ) < fItemDropRate )	// ¾ÆÀÌÅÛÀ» µå·ÓÇÒÁö ¸»Áö °áÁ¤. ·¹º§Â÷°¡ ¸¹ÀÌ ³ª¸é ¾Æ¿¹ ¶³¾îÆ®¸®Áö ¾Ê´Â´Ù.
			{
				...
				for( int i = 0; i < nSize; i++ )
				{
					if( ( lpDropItem = lpMoverProp->m_DropItemGenerator.GetAt( i, bUnique, GetPieceItemDropRateFactor( pAttacker ) ) ) != NULL )
					{
						if( lpDropItem->dtType == DROPTYPE_NORMAL )
						{
							...
							vPos.y = GetPos().y;
					#ifdef __EVENT_MONSTER
							// ÀÌº¥Æ® ¸ó½ºÅÍ°¡ µå¶øÇÑ ¾ÆÀÌÅÛÀº ¸ó½ºÅÍÀÇ ID¸¦ ±â¾ïÇÑ´Ù(ÆêÀÌ ¸ø ÁÝ°Ô...)
							if( CEventMonster::GetInstance()->SetEventMonster( lpMoverProp->dwID ) )
							{
							...
							if( pItemProp )
							{
								...
								if( pItemProp->dwItemKind1 == IK1_WEAPON || pItemProp->dwItemKind1 == IK1_ARMOR || ( pItemProp->dwItemKind1 == IK1_GENERAL && pItemProp->dwItemKind2 == IK2_JEWELRY ) )
								{
									switch( pItemProp->nLog )
									{
									case 1:		g_dpDBClient.SendLogUniqueItem( pAttacker, pItem, pItemElem->GetAbilityOption() );	// ÀÏ¹Ý ¾ÆÀÌÅÛ +¾ÆÀÌÅÛ
										break;
						...
						{
							int	nSeedID = 0;
							int nNumGold = lpDropItem->dwNumber + xRandom( lpDropItem->dwNumber2 - lpDropItem->dwNumber );	// Number ~ Number2 »çÀÌÀÇ ·£´ý°ª.
							...
							{
								// µ·¾×¼ö¿¡ µû¶ó ¾î¶²¸ð¾çÀÇ ½Ãµå¸¦ »ç¿ëÇÒÁö °áÁ¤ÇÑ´Ù.
				...
				for( i = 0; i < nSize; i++ )
				{
					...
					if (lpMoverProp->dwClass == RANK_BOSS  && nCurr == i)
					{
						if (bHasDroped == FALSE)
						{
							nAbilityOption = 1;
...
// this¿ÀºêÁ§Æ®¿¡´Ù ÂòÇÑ»ç¶÷(JJimer -_-;;;) ÀÇ ¾ÆÀÌµð¸¦ ¹Ú¾ÆµÖ¼­ 
...
void CMover::ProcessRecovery()
{
	...
	if( dwCurTick > m_dwTickDuel )	// µÎ¾ó ½ÃÀÛÈÄ 1ÃÊ¿¡ 3, 2, 1, Fight
	{
		...
		{
			--m_nDuelState;
			if( 100 == m_nDuelState )	// ¸¶Áö¸· Fight ¸Þ¼¼Áö¸¦ º¸³¿
			{
			...
			if( pParty->m_nKindTroup == 1 && pParty->m_nModeTime[PARTY_STRETCHING_MODE] )
			{
				...
				if( nMItem )
				{
					if( IsValidObj( pLeader ) && (
							pLeader->GetJob() == JOB_ASSIST ||
							pLeader->GetJob() == JOB_RINGMASTER ||
							pLeader->GetJob() == JOB_BILLPOSTER ||
							pLeader->GetJob() == JOB_RINGMASTER_MASTER ||
							pLeader->GetJob() == JOB_BILLPOSTER_MASTER ||
							pLeader->GetJob() == JOB_RINGMASTER_HERO ||
							pLeader->GetJob() == JOB_BILLPOSTER_HERO ||
							pLeader->GetJob() == JOB_FLORIST_HERO ||
							pLeader->GetJob() == JOB_FORCEMASTER_HERO 
							)
						)
						fRecovery = 1.8f;
					else
				...
				if( IsValidObj( (CObj*)pLeader ) )
				{
					...
					if( nMItem )
					{
						if(	pLeader->GetJob() == JOB_ASSIST || pLeader->GetJob() == JOB_RINGMASTER || pLeader->GetJob() == JOB_BILLPOSTER ||
							pLeader->GetJob() == JOB_RINGMASTER_MASTER || pLeader->GetJob() == JOB_BILLPOSTER_MASTER || pLeader->GetJob() == JOB_RINGMASTER_HERO ||
							pLeader->GetJob() == JOB_BILLPOSTER_HERO || pLeader->GetJob() == JOB_FLORIST_HERO || pLeader->GetJob() == JOB_FORCEMASTER_HERO 
							)
							fRecovery = 1.8f;
						else
							fRecovery = 1.5f;
...
void CMover::AllocShopInventory( LPCHARACTER pCharacter )
{
	if( IsVendorNPC() )
	{
		m_ShopInventory[ 0 ] = new CItemContainer< CItemElem  >; 
		m_ShopInventory[ 0 ]->SetItemContainer( ITYPE_ITEM, MAX_VENDOR_INVENTORY );
		m_ShopInventory[ 1 ] = new CItemContainer< CItemElem  >; 
		m_ShopInventory[ 1 ]->SetItemContainer( ITYPE_ITEM, MAX_VENDOR_INVENTORY );
		m_ShopInventory[ 2 ] = new CItemContainer< CItemElem  >; 
		m_ShopInventory[ 2 ]->SetItemContainer( ITYPE_ITEM, MAX_VENDOR_INVENTORY );
		m_ShopInventory[ 3 ] = new CItemContainer< CItemElem  >; 
		m_ShopInventory[ 3 ]->SetItemContainer( ITYPE_ITEM, MAX_VENDOR_INVENTORY );
	}
}
...
BOOL CMover::IsVendorNPC()
{
	...
	for( int i = 0; i < MAX_VENDOR_INVENTORY_TAB; i++ )
	{
		...
		if( pCharacter->m_nVenderType == 1 && pCharacter->m_venderItemAry2[i].GetSize() )
			return TRUE;
	}
...
#ifdef __WORLDSERVER
// È­»ìÀ» nCount´ë·Î »èÁ¦
// È­»ìÀ» ÀåÂøµÇ¾î ÀÖ´Â°ÍÀ» ¼Ò¸ðÇßÀ»½Ã ÀÚµ¿À¸·Î °°Àº È­»ìÀ» ÀåÂø
// °°Àº È­»ìÀÎÁö Ã¼Å©
void CMover::ArrowDown( int nCount )
{
	...
	if( pItemElem )
	{
		// ÀåÂøµÇ¾î ÀÖ´Â°Í ¸ðµÎ ¼ÒÁø µÇ¾úÀ½. ÀåÂøµÇ¾î ÀÖ´Â ¾ÆÀÌÅÛ »èÁ¦ÈÄ
...
{ 
#ifdef __CLIENT
	// ÀÌ¹Ì »ç¶÷ ¸ð¾çÀÌ¹Ç·Î ÇØÁ¦ Ã³¸® ÇÒ ÇÊ¿ä ¾øÀ½. 
...
{ 
	...
	if( m_bPlayer )
	// ÇÃ·¹ÀÌ¾îÀÏ °æ¿ì¿¡¸¸ ÀåÂøÀ» Àû¿ë. ÀÏ¹Ý npc¿¡ ÀåÂøÀ» Àû¿ëÇÏ¸é ¿©·¯°¡Áö Á¶°Ç °Ë»ç ³¡¿¡ Á÷¾÷, 
...
#ifdef __CLIENT
// pModel¸¦ ÀÎÀÚ·Î ¹ÞÀº ÀÌÀ¯´Â....±Û·Î¹ú¸ðµ¨À» »ç¿ëÇÏ´Â ³ÑµéÀÌ ÀÖ¾î¼­...
...
void CMover::AddSkillPoint( int nPoint )
{
	...
	if( nMaxPoint < m_nSkillLevel + nPoint )	// ´õ ÇØÁö¸é ³Ñ´Â °æ¿ì(ÇöÀç ³ÑÁö ¾ÊÀ½)
		nPoint = nMaxPoint - m_nSkillLevel;	// ¸Æ½ºÄ¡¿¡ ¸ðÀÚ¸¥ ¸¸Å­¸¸ Æ÷ÀÎÆ® Áö±Þ
...
BOOL CMover::IsStateDbuff()
{
	...
	if( nAdjParam & CHS_STUN || nAdjParam & CHS_POISON || nAdjParam & CHS_DARK ||
		nAdjParam & CHS_GROGGY || nAdjParam & CHS_SILENT || nAdjParam & CHS_BLEEDING || nAdjParam & CHS_SLEEPING )
#else
	if( nAdjParam & CHS_STUN || nAdjParam & CHS_POISON || nAdjParam & CHS_DARK ||
...
#endif // __VER >= 8 // __S8_PK
		// ¼öÁý ¾ÆÀÌÅÛ °¹¼ö  17 + MAX_QUESTCONDITEM
...
#endif	// __VER >= 8 // __S8_PK
		{
			// ¸ðµç Á¶°Ç ´Þ¼º ¼ø°£¿¡ Å¸ÀÌ¸Ó Ä«¿îÆ® ÁßÁö ½ÃÅ°±â 
...
int __IsBeginQuestCondition( CMover* pMover, int nQuestId )
{
	...
	if( pQuestProp )
	{
		...
		// Äù½ºÆ®°¡ Á¸ÀçÇÏ°Å³ª, ÀÌ¹Ì ¿Ï·áµÈ ÀûÀÌ ÀÖ´Â Äù½ºÆ®´Â Á¶°Ç ¼º¸³ ¾ÈµÊ. ¹Ýº¹ÇÏ±â À§ÇØ¼­´Â ¿ÏÀüÈ÷ Á¦°ÅÇÏ¿© ¿Ï·á ¹è¿­¿¡ ¾ø¾î¿© ÇÑ´Ù.
		...
		// ¼öÁý ¾ÆÀÌÅÛ °¹¼ö  21 + MAX_QUESTCONDITEM
...
{
#ifdef __CLIENT
	// m_dwPetId °¡ Á¸ÀçÇÏ´Âµ¥, m_pPetÀÌ ¾øÀ¸¸é, »ý¼ºÇÑ´Ù.
	// »ý¼º½Ã, ActiveMoverÀÎ °æ¿ì¿Í, ¾Æ´Ñ°æ¿ì¸¦ ºÐ¸®ÇØ¼­ Ã³¸®ÇÑ´Ù.
...
{
#ifdef __CLIENT
	if( m_pet.GetObj() )	// Á¦°Å ÇÏ¸é, ÇÁ·Î¼¼½º¿¡¼­ Àç»ý
	{
...
//#endif	// __INTERNALSERVER
		...
		)
	{
		...
		for( int i = 0 ; i < nMax; i++ )
		{
			...
			{
				if( ( m_nCount & 0X1F ) == 0 )	// 2ÃÊ´ç ¸ÔÀÌ 1¼Ò¸ð		// 0723
...
//#else	// __INTERNALSERVER
	if( ( m_nCount & 1023 ) != 0 )	// // °æÇèÄ¡°¡ 1ºÐ¿¡ 1È¸ »ó½Â
//#endif	// // °æÇèÄ¡°¡ 1ºÐ¿¡ 1È¸ »ó½Â
		return;
...
//1/3	HI_COUNT_CHECK	HS_PVP_POINT04	4000	IDS_TITLE_TXT_0014	//ÆÐ±âÀÖ´Â ÆÄÀÌÅÍ			//	ÆÐ±âÀÖ´Â ÆÄÀÌÅÍ
...
#ifdef __WORLDSERVER
	// m_dwHonorCheckTime
	if( m_pActMover->GetActionState() == OBJSTA_COLLECT )	//Ã¤ÁýÁß
	{
		DWORD dwTick = GetTickCount();
...
CItemElem* CMover::GetOldCollector( void )
{
	CItemElem* pCol		= GetWeaponItem();
	if( pCol && ( !pCol->IsOldCollector() || pCol->IsFlag( CItemElem::expired ) ) )
		pCol	= NULL;
	return pCol;
}
#endif // __V6_COLLECTION_SYSTEM
...
// ¼ÒÀ¯ÇÏ°íÀÖ´Â Æä¸°°ú Æä³ÄÀÇ ÇÕÀ» ¹ÝÈ¯ÇÑ´Ù.
...
// iGold¸¸Å­ÀÇ Æä¸°°ú Æä³Ä¸¦ ¼Ò¸ðÇÏ°í ÀÌ¸¦ Å¬¶óÀÌ¾ðÆ® Åëº¸ÇÑ´Ù.
// Ë¬ 2008-10-08: ¼öÁ¤
...
#ifdef __CHANGEJOB_RAPIDE
/*void CMover::InitLevelafterJobChange( int nJob, LONG nLevel )
{
#ifdef __WORLDSERVER
	...
	if( pProp )
	{
		...
		else if( nJob > 0 && (nJob < MAX_PROFESSIONAL || nJob >= MAX_MASTER) && nJob < MAX_LEGEND_HERO )
			AddChangeJob( nJob );
		else
			return;
		...
		if( nJob == JOB_MERCENARY )
			nPoint = 40;
		else if( nJob == JOB_ACROBAT )
			nPoint = 50;
		else if( nJob == JOB_ASSIST )
			nPoint = 60;
		else if( nJob == JOB_MAGICIAN )
			nPoint = 90;
		else if( nJob ==  JOB_BILLPOSTER || nJob ==  JOB_RINGMASTER )
			nPoint = 120;
		else if( nJob ==  JOB_KNIGHT || nJob ==  JOB_BLADE )
			nPoint = 80;
		else if( nJob ==  JOB_JESTER || nJob ==  JOB_RANGER  )
			nPoint = 100;
		else if( nJob ==  JOB_ELEMENTOR || nJob ==  JOB_PSYCHIKEEPER )
			nPoint = 90;
		else if( nJob >= MAX_HERO )
			nPoint = 16;
		else if( nJob < MAX_MASTER && nJob >= MAX_PROFESSIONAL )
		{
			for( int i=0;i<MAX_GENERAL_LEVEL;++i )
			{
				nAddPoint += ((i - 1) / 20) + 2;	//Skillpunktformel aus CMover::AddExperience()
			}

			if( nJob == JOB_BLADE_MASTER )
			{
				nAddPoint += (40 + 80);
				nPoint = 870 - nAddPoint;
			}
			else if( nJob == JOB_KNIGHT_MASTER )
			{
				nAddPoint += (40 + 80);
				nPoint = 870 - nAddPoint;
			}
			else if( nJob == JOB_RINGMASTER_MASTER )
			{
				nAddPoint += (60 + 120);
				nPoint = 950 - nAddPoint;
			}
			else if( nJob == JOB_BILLPOSTER_MASTER )
			{
				nAddPoint += (60 + 120);
				nPoint = 950 - nAddPoint;
			}
			else if( nJob == JOB_RANGER_MASTER )
			{
				nAddPoint += (50 + 100);
				nPoint = 910 - nAddPoint;
			}
			else if( nJob == JOB_JESTER_MASTER )
			{
				nAddPoint += (50 + 100);
				nPoint = 910 - nAddPoint;
			}
			else if( nJob == JOB_PSYCHIKEEPER_MASTER )
			{
				nAddPoint += (90 + 90);
				nPoint = 950 - nAddPoint;
			}
			else if( nJob == JOB_ELEMENTOR_MASTER )
			{
				nAddPoint += (90 + 90);
				nPoint = 1280 - nAddPoint;
			}
		}
		...
		if( nJob >= MAX_JOBBASE && nJob < MAX_EXPERT )
		{
			m_nStr = m_nSta = m_nDex = m_nInt = 15;
			m_nRemainGP = 28;
		}
		else if( nJob < MAX_HERO && nJob >= MAX_MASTER )
		{
			m_nRemainGP += 15;
		}
		else if( nJob < MAX_MASTER && nJob >= MAX_PROFESSIONAL )
		...
		CItemElem itemelem[4];
		for( int i=0;i<4;i++ )
		{
			itemelem[i].m_nItemNum = 1;
			itemelem[i].m_bCharged = TRUE;
		}

		if( nJob == JOB_MENTALIST_HERO )
		{
			itemelem[0].m_dwItemId = II_WEA_BOOK_BOKROMAIN;
		}
		else if( nJob == JOB_FORCEMASTER_HERO )
		{
			itemelem[0].m_dwItemId = II_ARM_ARM_SHI_ZEMBATO;
		}
		else if( nJob < MAX_EXPERT )
		{
			itemelem[1].m_dwItemId = II_SYS_SYS_SCR_RECCURENCE;
			( ( CUser*)this)->CreateItem( &itemelem[1] );
			itemelem[2].m_dwItemId = II_CHR_FOO_COO_BULLHAMS;
			( ( CUser*)this)->CreateItem( &itemelem[2] );
			itemelem[3].m_dwItemId = II_CHR_SYS_SCR_ACTIVITION;
			( ( CUser*)this)->CreateItem( &itemelem[3] );
			if( nJob == JOB_ASSIST || nJob == JOB_MAGICIAN )
				itemelem[0].m_dwItemId = II_CHR_REF_REF_HOLD;
			else if( nJob == JOB_MERCENARY || nJob == JOB_ACROBAT )
				itemelem[0].m_dwItemId = II_CHR_POT_DRI_VITALX;
		}

		if( itemelem[0].m_dwItemId != NULL )
			( ( CUser*)this)->CreateItem( &itemelem[0] );
