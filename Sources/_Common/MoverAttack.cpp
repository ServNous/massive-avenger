...
BOOL CMover::IsBackSide( CMover* pMover )
{
	D3DXVECTOR3 v3Pos;
	...
	// 방향벡터 1
	v3PosSrc = pMover->GetPos() - GetPos();
	D3DXVec3Normalize( &v3PosSrc, &v3PosSrc );

	// 방향벡터 2
	AngleToVectorXZ( &v3Pos, pMover->GetAngle(), 3.0f );
	v3PosDest = (pMover->GetPos()+v3Pos) - pMover->GetPos();
...
BOOL CMover::GetAttackResult( CMover* pDefender, DWORD dwOption )
{
	...
	{
#if __VER <= 9    // 	__PVPDEMAGE0608	//	9th PVP DEMAGE edit // Inversion >= 9 par ToX82
...
int CMover::CalcDefenseNPC( CMover* pAttacker, DWORD dwAtkFlags )
{
	...
	MoverProp* pProp = GetProp();
	if( !pProp || !pAttacker || !dwAtkFlags )
	{
	Error( "[ERREUR] CMover::CalcDefenseNPC -> pProp = %s / pAttacker = %s / dwAtkFlags = %s", pProp, pAttacker, dwAtkFlags );
	fDefense = 7.0f + 1;
    return (int)fDefense;
	}
...
float CMover::GetDamageMultiplier( ATTACK_INFO* pInfo )
{
	    ...
	    if( nSkill == SI_PSY_PSY_SPRITBOMB )
    {
            ...
            factor = factor * 2.0f * (fPercent / 100.0f);
    }
...
int CMover::GetMagicSkillPower( ATTACK_INFO* pInfo )
{
	...
	nATK += (int)( nATK * GetAdjParam( DST_CAST_CRITICAL_RATE ) / 100 );
	...
	switch( pSkillProp->dwSpellType )
	{
		...
		case ST_EARTH:	
			...
			nATK	= (int)( nATK + (nATK * fRatio) );
			break;
#ifdef __SEY_NEW_ELEM_DMG_CALC
		case ST_FIREEARTH:
			fRatio	= ( GetParam( DST_MASTRY_FIRE, 0 ) + GetParam( DST_MASTRY_EARTH, 0 ) ) / 100.0f;
			nATK	= (int)( nATK + (nATK * fRatio) );
			break;
		case ST_ELECWIND:
			fRatio	= ( GetParam( DST_MASTRY_ELECTRICITY, 0 ) + GetParam( DST_MASTRY_WIND, 0 ) ) / 100.0f;
			nATK	= (int)( nATK + (nATK * fRatio) );
			break;
		case ST_EARTHWIND:
			fRatio	= ( GetParam( DST_MASTRY_EARTH, 0 ) + GetParam( DST_MASTRY_WIND, 0 ) ) / 100.0f;
			nATK	= (int)( nATK + (nATK * fRatio) );
			break;
		case ST_EARTHWATER:
			fRatio	= ( GetParam( DST_MASTRY_EARTH, 0 ) + GetParam( DST_MASTRY_WATER, 0 ) ) / 100.0f;
			nATK	= (int)( nATK + (nATK * fRatio) );
			break;
#endif // __SEY_NEW_ELEM_DMG_CALC
	}
...
float CMover::GetMagicSkillFactor( CMover* pDefender, SAI79::ePropType skillType )
{
	...
	if( ( skillType == itemType ) ||
		( ( skillType == 6 ) && ( ( itemType == 1 ) || ( itemType == 5 ) ) ) ||
		( ( skillType == 7 ) && ( ( itemType == 3 ) || ( itemType == 4 ) ) ) ||
		( ( skillType == 8 ) && ( ( itemType == 5 ) || ( itemType == 4 ) ) ) ||
		( ( skillType == 9 ) && ( ( itemType == 5 ) || ( itemType == 2 ) ) ) )
	{
		if( skillType == 0 )
			return 1.0f;
		else
		{
			float fMult = (float)( pWeapon->m_nResistAbilityOption / 100 ) + 1.1f;
			return fMult;
		}
	}
...
HITTYPE	CMover::GetHitType( CMover* pMover, BOOL bTarget, int nReflect )
{
	...
	if( IsPlayer() && pMover->IsPlayer() )
	{
		if( g_eLocal.GetState( EVE_GUILDWAR ) )       // 길드전 상태에섦� PK불가
     {
...
BOOL  CMover::IsArenaTarget( CMover* pMover )
{
   ...
   if( pWorld && pWorld->IsGwar() )
     return FALSE;

   if ( pMover->IsRegionAttr( RA_FIGHT ) || pMover->IsRegionAttr( RA_PK ) || pMover->IsRegionAttr( RA_DANGER ) )
     return TRUE;
...
{ 
	...
	switch( type )
	{
		case SAI79::FIRE:			return GetResistSpell( DST_RESIST_FIRE ) / 100.0f;
		case SAI79::WATER:			return GetResistSpell( DST_RESIST_WATER ) / 100.0f;
		case SAI79::ELECTRICITY:	return GetResistSpell( DST_RESIST_ELECTRICITY ) / 100.0f;
		case SAI79::WIND:			return GetResistSpell( DST_RESIST_WIND ) / 100.0f;
		case SAI79::EARTH:			return GetResistSpell( DST_RESIST_EARTH ) / 100.0f;
#ifdef __SEY_NEW_ELEM_DMG_CALC
		case SAI79::FIREEARTH:		return ( GetResistSpell( DST_RESIST_FIRE ) + GetResistSpell( DST_RESIST_EARTH ) ) / 100.0f;
		case SAI79::ELECWIND:		return ( GetResistSpell( DST_RESIST_ELECTRICITY ) + GetResistSpell( DST_RESIST_WIND ) ) / 100.0f;
		case SAI79::EARTHWIND:		return ( GetResistSpell( DST_RESIST_EARTH ) + GetResistSpell( DST_RESIST_WIND ) ) / 100.0f;
		case SAI79::EARTHWATER:		return ( GetResistSpell( DST_RESIST_EARTH ) + GetResistSpell( DST_RESIST_WATER ) ) / 100.0f;
#endif // __SEY_NEW_ELEM_DMG_CALC
		default:					return 0.0f;
	}
}
