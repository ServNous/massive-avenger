...
CMclCritSec	s_cs;
#endif	// __SYS_PLAYER_DATA
CDbManager & g_DbManager = CDbManager::GetInstance();							// CDbManager Å¬·¡½º »ý¼º
...
void CDbManager::CreatePlayer( CQuery *qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
   ...
   {
     CString strPacket;
     strPacket.Format( "[DSystem] User: %s -> Type: Hack du 4ieme slot !", lpDbOverlappedPlus->AccountInfo.szAccount);
...
// ÇÃ·¹ÀÌ¾î ¸®½ºÆ®¸¦ º¸³¿(ÇÑ°èÁ¤¿¡ ÀÖ´Â ¸ðµç ÇÃ·¹ÀÌ¾î)
...
BOOL CDbManager::OpenGuildCombat( void )
{
	...
	if( qry.Fetch() )
	{
		...
		// Áö±Ý ±îÁö º¸»ó ¹× Âü¿©±ÝÀ» ¾È¹ÞÀº ±æµå ¹× º£½ºÆ® ÇÃ·¹ÀÌ¾î ¾ò¾î¿À±â
...
void CDbManager::SaveMessengerFriend( CQuery *qry, CMover* pMover, char* szQuery )
{
	for( C2FriendPtr::iterator i	= pMover->m_Messenger.m_aFriend.begin(); i != pMover->m_Messenger.m_aFriend.end(); ++i )
	{
		...
		if( pFriend->bSave )
		{
			// ¸Þ½ÅÀú Ãß°¡½Ã ¼ºº°ÇÏ°í Á÷¾÷Àº DB¿¡¼­ ÀÚÃ¼ÀûÀ¸·Î JoinÀ» ÇØ¼­ ¼ºº°°ú Á÷¾÷À» °¡Áö°í ¿È
			sprintf( szQuery,
...
//		nYear = pItemElem->m_dwKeepTime / 100000000;	dwTemp = pItemElem->m_dwKeepTime % 100000000;
	...
	{
		for( int i=0; i<pItemElem->GetPiercingSize(); i++ )		// ÀÏ¹Ý ¾ÆÀÌÅÛ
		{
		...
		for( i=0; i<pItemElem->GetUltimatePiercingSize(); i++ )		// ¾óÅÍ¸Ú ¿þÆù
		{
...
SERIALNUMBER CDbManager::SendItem( CQuery *pQry, char* szSql, CMover* pMover, __SendItemContents SendItemContents )
{
	...
	// ¸¸¾à °­È­ ¾ÆÀÌÅÛÀ» ³Ö¾î¾ß ÇÑ´Ù¸é, Item_Send Å×ÀÌºí¿¡ °­È­ Á¤µµ¸¦ ³ªÅ¸³»´Â ÄÃ·³À» Ãß°¡ÇÏ¿© ÀÐÀº ÈÄ, ÀÌ ÇÔ¼ö ¸¶Áö¸· ÀÎÀÚ(ÇöÀç 0)¸¦  ±× °ªÀ¸·Î ´ëÃ¼ÇÑ´Ù.
	if( pItemProp )
	{
		iSerialNumber	= CSerialNumber::GetInstance()->Get();
...
int CDbManager::SelectTag( CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus, u_long uidPlayer, TAG_ENTRY* tags)
{
	...
	while( qry->Fetch() )
	{
		...
		qry->GetStr( "m_Message", pEntry->szString );	// ¸Þ¼¼Áö
...
/*
	[200407_1_SCHOOL_TBL]  ÀúÀåÇÏ±â
	 ...
	 [200407_1_SCHOOL_STR] 1,'´ë¿ø¿Ü±¹¾î°íµîÇÐ±³',12
*/
...
void CDbManager::SerializeWanted( CQuery* pQuery, CAr& out )
{
	...
	while( pQuery->Fetch() )
	{
		...
		penya = pQuery->GetInt64( "penya" );			// Çö»ó±Ý¾×
...
BOOL CDbManager::OpenWanted( CAr& ar )
{
	...
	if( qry.Connect( 3, DSN_NAME_CHARACTER01, DB_ADMIN_ID_CHARACTER01, DB_ADMIN_PASS_CHARACTER01 ) == FALSE )
	{
		AfxMessageBox( "Error : Connection à la BDD CHARACTER01 immposible !" );
		return FALSE;
	}
...
//	@iGu = 'A1' Çö»ó±Ý °É±â (@m_idPlayer,@serverindex,@penya,@szMsg )
//	@iGu = 'D1' Çö»ó¹ü Á¦°Å (@m_idPlayer,@serverindex)
//	Çö»ó±Ý ½Ã½ºÅÛ µðºñ Ã³¸®¸¦ ÇÑ´Ù.
...
void CDbManager::InGuildCombat( CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
	...
	{	WriteLog( "InGuildCombat()"); TRACE("ERROR: InGuildCombat() - uspJoinGuildToCombat\n"); }
...
void CDbManager::OutGuildCombat( CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
	...
	{	WriteLog( "OutGuildCombat()");	TRACE("ERROR: OutGuildCombat() - uspCancelGuildToCombat\n");	}
...
void CDbManager::StartGuildCombat( CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
	...
	for( DWORD veci = 0 ; veci < m_GCResultValueGuild.size() ; ++veci )
	{
		...
		if( FALSE == pQuery->Exec( szQuery ) )
		{	WriteLog( "StartGuildCombat()");	TRACE("ERROR: StartGuildCombat() - uspExpireCombatReward\n");	}
	}
	m_GCResultValueGuild.clear();
...
//				m_uBestPlayer = uidPlayer;
	...
	// º¸»ó ¹× Âü¿©±ÝÀ» ¾È¹ÞÀº ±æµå ¹× º£½ºÆ® ÇÃ·¹ÀÌ¾î ¾ò¾î¿À±â
	m_GCResultValueGuild.clear();
	m_GCResultValuePlayer.clear();
	sprintf( szQuery, "uspLoadCombatUnpaidList '%02d'", g_appInfo.dwSys );
	if( FALSE == pQuery->Exec( szQuery ) )
...
BOOL CDbManager::CanJoin( LPCTSTR szAccount, ACCOUNT_CACHE* pAccountCache )
{
	...
	if( pAccountCache->IsPlaying() )
	{
		...
		// Á¾·á ÈÄ ½Ã°£ÀÌ 3ºÐ ÀÌ»ó °æ°ú µÇ¾ú´Ù¸é Çã¿ëÇÑ´Ù.
	...
	if( t > pAccountCache->m_tmPrevent )	// ÀÏ¹Ý
		return TRUE;
	else
	{
		char szBuffer[256];
...
//			GetBankMover( pMover, pQueryLoad, 0 );
			...
			if( FALSE == pQuerySave->Exec( szSql ) )
			{
				...
				SAFE_DELETE( pQuerySave );
				SAFE_DELETE_ARRAY( szSql );
				AfxMessageBox( "UPDATE CHARACTER_TBL", MB_OK );
				return FALSE;
			}
			...
			if( GetInventory( pMover, pQueryLoad, lpDbOverlappedPlus ) == FALSE )
			{
				...
				SAFE_DELETE( pQuerySave );
				SAFE_DELETE_ARRAY( szSql );
				AfxMessageBox( "insert ITEM_SEND_TBL : GetInventory", MB_OK );
				return FALSE;
			}
			// mirchang_100416
						...
						for( int j = 0; j < pItemElem->GetPiercingSize(); j++ )
						{
							if( pItemElem->GetPiercingItem( j ) > 0 )
							{
								...
								if( FALSE == pQuerySave->Exec( szSql ) )
								{
									...
									SAFE_DELETE( pQuerySave );
									SAFE_DELETE_ARRAY( szSql );
									AfxMessageBox( "insert ITEM_SEND_TBL : GetPiercingItem", MB_OK );
									return FALSE;
								}
							}
						}
...
//					TRACE( "%s\n", szSql );
					if( FALSE == pQuerySave->Exec( szSql ) )
					{
						...
						SAFE_DELETE( pQuerySave );
						SAFE_DELETE_ARRAY( szSql );
						AfxMessageBox( "insert ITEM_SEND_TBL : pQuerySave->Exec( szSql )", MB_OK );
						return FALSE;
					}
				...
				if( it != mRestore.end() )
				{
					if( pItemElem->m_pPet )
					{
						...
						if( pItemElem->IsFlag( CItemElem::expired ) )
						{
							...
							nLife--;	// »ç¸ÁÀÌ ¸®¼ÂµÇ¹Ç·Î »ý¸í 1Ãß°¡¿Í °°´Ù.
						}
