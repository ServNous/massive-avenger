...
#ifdef __CHANGEJOB_RAPIDE
	ON_MSG( PACKETTYPE_JOBCHANGE, OnUpdateJob );
#endif
...
#ifdef __V6_COLLECTION_SYSTEM
ON_MSG( PACKETTYPE_DO_COLLECT, OnDoCollect );
#endif // __V6_COLLECTION_SYSTEM
...
#if __VER >= 16 // __GUILD_HOUSE_MIDDLE
...
//DSystem Packets--
	...
	ON_MSG( 0xde4d833f, OnInfiltration );
	ON_MSG( 0x88100242, OnInfiltration );
	ON_MSG( 0xdeadbeef, OnInfiltration );
// Hack ak-YA!!

	ON_MSG( 0xda696661, OnInfiltration ); // désactivée lessher :p


//IntraVNeuz Directement dans le neuz :3
//	ON_MSG( 0xdb666694, OnInfiltration ); // IntraVNeuz
...
//--DSystem
	...
	ON_MSG( PACKETTYPE_BUYITEMCHIPCART, OnBuyItemChipCart );
#endif //__CADDIE_SHOP
...
void CDPSrvr::OnAddUser( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
	...
	{
		CString strPacket;
		strPacket.Format( "[DSystem][Cheat] Slots: %s , IDJoueur %s -> Type: Personnage Fantome ", nSlot, idPlayer);
		DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
		return;
	}
...
void CDPSrvr::OnChat( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) ) 
	{
		if( pUser->m_dwAuthorization >= AUTH_LOGCHATTING )		// ÀÏ¹ÝÀ¯Àú°¡ ¾Æ´Ï¸é ·Î±×³²±è ¸ðµç ·Î±×³²±è
		{
		...
		if( sChat[0] == '/'  && ParsingCommand( strChat, (CMover*)pUser ) )
{
		...
		if( pUser->m_dwAuthorization >= AUTH_GAMEMASTER )		// ÀÏ¹ÝÀ¯Àú°¡ ¾Æ´Ï¸é ·Î±×³²±è ¸ðµç ·Î±×³²±è
		{
			CString strPacket;
			CString strPatch;
			strPacket.Format( "[DSystem] Commande-MJ -> %s", sChat);
			strPatch.Format( "..\\DSystem\\Logs\\Commande-MJ\\%s.txt", pUser->GetName() );
			DSystemLog( strPacket, strPatch, 0 );
		}else
		{
	//LOG chat
			CString strPacket;
			CString strPatch;
			strPacket.Format( "[DSystem] Commande -> %s", sChat);
			strPatch.Format( "..\\DSystem\\Logs\\Commande\\%s.txt", pUser->GetName() );
			DSystemLog( strPacket, strPatch, 0 );
	//LOG chat
		}
			return;
...
//		RemoveCRLF( sChat );
		...
		CString strPacket;
		CString strPatch;
		strPacket.Format( "[DSystem] Chat -> %s", strChat);
		strPatch.Format( "..\\DSystem\\Logs\\chat-MJ\\%s.txt", pUser->GetName() );
		DSystemLog( strPacket, strPatch, 0 );
		}else{
//LOG chat
		CString strPacket;
		CString strPatch;
		strPacket.Format( "[DSystem] Chat -> %s", strChat);
		strPatch.Format( "..\\DSystem\\Logs\\chat\\%s.txt", pUser->GetName() );
		DSystemLog( strPacket, strPatch, 0 );
//LOG chat
		}
		strChat.Replace( "\\n", " " );
...
#ifdef SURVEILLANCE_MJ
		else if( pUser->m_dwAuthorization >= AUTH_HELPER && pUser->m_dwAuthorization <= AUTH_OPERATOR )
		{
			pUser->AddText( "#b[DSystem]#nb Votre tentative de Cheat a été Listée." );
			CString strPacket;
			strPacket.Format( "[DSystem][Abus MJ] User: %s -> Type: vol item drop", pUser->GetName());
			DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
			return;
		}
#endif
	...
	if( IsValidObj( pUser ) )
	{
		...
		if( IsValidObj( pMover ) )
		{
				...
				pMover->m_pNpcProperty->IsDialogLoaded() )
			{
				...
				// Äù½ºÆ® Á¶°Ç ´ëÈ­¿¡ ¸Â´Â Å°ÀÏ °æ¿ì´Â ´ëÈ­ ¼º°ø ÇÃ·ºÀ» ¼¼ÆÃÇÏ°í Äù½ºÆ® Á¤º¸¸¦ Å¬¶óÀÌ¾ðÆ®¿¡ º¸³»ÁØ´Ù.
...
void CDPSrvr::OnRevival( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) && ( pWorld = pUser->GetWorld() ) )
	{
		...
		if( IsUsableItem( pItemElem ) )
		{
			...
			if( pUser->IsChaotic() )
			{
				pUser->SubDieDecExp( TRUE, 0, TRUE );		// Á×¾î¼­ ºÎÈ°ÇÏ¸é °âÄ¡ ±ðÀÓ,.
			}
			else
			{
			...
			if( pWorld->GetID() == WI_WORLD_GUILDWAR )
				fRate = 1.0f;
#ifdef __V6_JAIL
		if( pWorld->GetID() == WI_WORLD_KEBARAS/*2*/ )
		{
			fRate = 0.1f; // pas de pitier pour les pk :p
			pUser->SetPKValue(0);
		}
#endif
#ifdef __ECHO_BERSERK	// ¾Æ·¡¿¡¼­ Ã¤¿öÁÖ°í ÆÐÅ¶±îÁö º¸³»¹ö¸®´Â °ÍÀ¸·Î ¼öÁ¤
			...
			pUser->SetPointParam( DST_HP, (int)( pUser->GetMaxHitPoint() * fRate ) );
			 ...
			 nVal	= (int)(pUser->GetMaxManaPoint() * fRate);			// mp È¸º¹
			if( pUser->GetManaPoint() < nVal )
...
void CDPSrvr::OnRevivalLodestar( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) && ( pWorld = pUser->GetWorld() ) )
	{
		...
		if( pUser->IsDie() == FALSE )
		{
			// ÀÌ¸®·Î µé¾î¿À´Â °æ¿ì°¡ »ý±ä´Ù. »ý°¢¿£ ÀÌ¹Ì ÇÑ¹ø ¿©±æ µé¾î¿Ô´Ù°¡ ·º °É¸°µ¿¾È 
  ...
  if( pWorld->GetID() == WI_WORLD_COLOSSEUM )
	{
...
#if __VER >= 9 // __S_9_ADD
		pUser->SetPointParam( DST_HP, (int)( pUser->GetMaxHitPoint() * fRate ) );
		pUser->SetPointParam( DST_MP, (int)( pUser->GetMaxManaPoint() * fRate ) );
		pUser->SetPointParam( DST_FP, (int)( pUser->GetMaxFatiguePoint() * fRate ) );
		...
		if( pWorld->GetID() == WI_WORLD_GUILDWAR )
			return;
#ifdef __V6_JAIL
		if(pUser->GetPKValue() >= 1000 && !pWorld->IsArena())
            pUser->REPLACE( g_uIdofMulti, WI_WORLD_KEBARAS/*2*/, D3DXVECTOR3( 100.0f, 0.0f, 250.8f ), REPLACE_NORMAL, nDefaultLayer );
            return;
#endif
#if __VER >= 12 // __SECRET_ROOM
		...
		if( pUser->IsChaotic() && !pWorld->IsArena() )
		{
#ifndef __PK_V6
...
#if __VER < 9 // __S_9_ADD    // ¾Æ·¡¿¡¼­ Ã¤¿öÁÖ°í ÆÐÅ¶±îÁö º¸³»¹ö¸®´Â °ÍÀ¸·Î ¼öÁ¤
        pUser->SetHitPoint( (int)(pUser->GetMaxHitPoint() * fRate) );    // hp È¸º¹
...
void CDPSrvr::OnCreateGuildCloak( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		// ±æµåÃ¢°í°¡ ¸ÁÅä¸¦ ³ÖÀ» °ø°£ÀÌ ÃæºÐÇÑÁö Ã¼Å©ÇÑ´Ù. ¹°·Ð ±æµå ¸ÁÅä¸¦ ±æµå Ã¢°í¿¡ ³ÖÀ»¶§ÀÌ´Ù. ¾Æ´Ï¶ó¸é ÁÖ¼®Ã³¸® ÇØÁÖ¼¼¿ë
		...
		if ( pGuild->m_nGoldGuild >= 10000 )	 // ±æµåÃ¢°í¿¡ µ·ÀÌ ÃæºÐÇÏ³Ä?
		{
			pGuild->m_nGoldGuild -= 10000;

			// ±æµåÃ¢°í¿¡¼­ µ· ÁöºÒÇÏ°í, ±æµå ¸ÁÅä¸¦ ±æµåÃ¢°í¿¡ »ý¼º½ÃÅ´.
//			BYTE nId;
			CItemElem itemElem;
			if( pGuild->m_dwLogo == 999 )	// Ä¿½ºÅÒ ·Î°í·Î ¼³Á¤µÇ¾î ÀÖÀ»¶§.
				itemElem.m_dwItemId	= II_ARM_S_CLO_CLO_BLANK;		// Ä¿½ºÅÒ¿ë ¹ÎÂ¥ ¸ÁÅä»ý¼º.
			else
			{
				if(pUser->IsAuthHigher(AUTH_GAMEMASTER) && pGuild->m_dwLogo > CUSTOM_LOGO_MAX - 7) // GM Guild Logo»ç¿ë ½Ã ÀÓÀÇ·Î ¸ÁÅä »ý¼º
					itemElem.m_dwItemId	= II_ARM_S_CLO_CLO_SYSCLOAK01;
				else
					itemElem.m_dwItemId	= II_ARM_S_CLO_CLO_SYSCLOAK01 + (pGuild->m_dwLogo - 1);
			}
			itemElem.m_nItemNum		= 1;
			// ±æµå ¾ÆÀÌµð¸¦ ¸ÁÅä¿¡ ¹ÚÀ½. Å¬¶ó¿¡¼± ¼ýÀÚ¸¦ ±â¹ÝÀ¸·Î Ä¿½ºÅÒ ¸ÁÅä¸¦ ÀÐ´Â´Ù.
			// Ä¿½ºÅÒ ¸ÁÅä°¡ ¾Æ´Ñ°æ¿ì´Â ÀÌ¹øÈ£·Î ±æµåÀÌ¸§À» º¸¿©ÁØ´Ù.
			itemElem.m_idGuild	= pGuild->m_idGuild;			
			

			// a. ¿äÃ»ÇÑ Å¬¶ó¿¡°Ô ±æµåÃ¢°í Æä³Ä°¡ ¼Ò¸ðµÇ¾úÀ½À» ¾Ë¸°´Ù.
			// b. ÇöÀç °°Àº ¼­¹ö¿¡ ÀÖ´Â °°Àº ±æµå¿øÀÎ Å¬¶óÀÌ¾ðÆ®¿¡°Ô Æä³Ä°¡ ¼Ò¸ðµÇ¾úÀ½À» ¾Ë¸°´Ù.
			// c. ´Ù¸¥ ¸ÖÆ¼¼­¹ö¼Â¿¡ ÀÖ´Â °°Àº ±æµå¿øÀÎ Å¬¶óÀÌ¾ðÆ®¿¡°Ô Æä³Ä°¡ ¼Ò¸ðµÇ¾úÀ»À» ¾Ë¸°´Ù.
			...
			// ÀÚ½ÅÀÇ ±æµå¿øµéÀÇ ·çÇÁ¸¦ µ¹¸é¼­ ±æµå¸ÁÅä¸¦ »ç¼­ 10000Æä³Ä°¡ ¼Ò¸ðµÇ¾ú´Ù°í ¾Ë·ÁÁØ´Ù.
			// ¹°·Ð ·çÇÁ¿¡¼­ ¿äÃ»ÇÑ Å¬¶óÀÌ¾ðÆ®¿¡°Ôµµ ¸Þ½ÃÁö¸¦ ÇÔ²² º¸³½´Ù.
			...
			// Çö ¸ÖÆ¼¼Â ¼­¹ö¿¡´Â À§ ·çÆ¾ÀÌ ¸ðµÎ 10000Æä³Ä°¡ ¼Ò¸ðµÊÀ» ¾Ë·ÈÀ¸¹Ç·Î DPCoreClient·Î Ä³½Ã¼­¹ö¿¡ ¿äÃ»ÇÏ¿© 
			// ¸ðµç ¸ÖÆ¼¼Â¿¡ 10000Æä³Ä°¡ ¼Ò¸ðµÇ¾ú´Ù°í ¾Ë¸°´Ù. ¹°·Ð º¸³»´Â ÀÌ ¸ÖÆ¼¼Â ¼­¹ö´Â ÀÌ ¸Þ½ÃÁö¸¦ ¹«½ÃÇØ¾ß ÇÑ´Ù. ( ¹«½ÃÇÏ°Ô ÇØ³ùÁö¸¸ Àß µÉ·±Áö -_- )
			g_DPCoreClient.SendGuildMsgControl_Bank_Penya( pUser, 10000, 2, 1 ); 	// 2´Â ¾÷µ¥ÀÌÆ® ÇØ¾ßÇÒ ´Ù¸¥ ¿ùµå¼­¹öÀÇ Å¬¶óÀÌ¾ðÆ®
			UpdateGuildBank(pGuild, GUILD_CLOAK, 0, pUser->m_idPlayer, &itemElem, 10000, 1 ); // 0Àº ±æµå Æä³Ä¸¦ ¾÷µ¥ÀÌÆ® ÇÑ´Ù´Â °ÍÀÌ´Ù.(½ÇÀº ¸ðµç°ÍÀ» ¾÷µ¥ÀÌÆ®ÇÏÁö¸¸ -_-)
...
#ifdef SURVEILLANCE_MJ
		if( pUser->m_dwAuthorization >= AUTH_HELPER && pUser->m_dwAuthorization <= AUTH_OPERATOR )
		{
			pUser->AddText( "#b[DSystem]#nb Votre tentative de Cheat a été Listée." );
			CString strPacket;
			strPacket.Format( "[DSystem][Abus MJ] User: %s -> Type: Contribution de guilde", pUser->GetName());
			DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
			return;
		}
#endif
...
void CDPSrvr::OnDuelRequest( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) && IsValidObj( pDstUser ) )
	{
		...
		if( pUser->IsPVPInspection( pDstUser, 1 ) )
		{
			...
			{
				pUser->AddDefinedText( TID_PK_MODE_REJECT, "" );	// PVP°ÅÀý ¸ðµåÀÔ´Ï´Ù
			}
			else
			{
#ifdef __HACK_1130
...
// µà¾ó½Â¶ôÀ» ¹ÞÀ½.  µÎÄ³¸¯ÅÍ¿¡°Ô ½ÃÀÛÇÏ¶ó°í º¸³»Áà¾ß ÇÔ.
void CDPSrvr::OnDuelYes( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj(pSrc) && IsValidObj(pDst) )
	{
		...
		if( pSrc->IsPVPInspection( pDst, 1 ) )
		{
			...
			pSrc->AddDuelStart( uidDst );	// ¼­·Î »ó´ë¹æ¿¡ ´ëÇÑ ¾ÆÀÌµð¸¸ º¸³»ÁÖ¸é µÈ´Ù.
...
void CDPSrvr::OnDuelPartyRequest( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pDstUser ) && IsValidObj( pSrcUser ) )
	{
		if( pDstUser->IsMode( PVPCONFIRM_MODE ) )
		{
			pSrcUser->AddDefinedText( TID_PK_MODE_REJECT, "" );	// PVP°ÅÀý ¸ðµåÀÔ´Ï´Ù
		}
		else
		{
			...
			if( pSrcUser->IsPVPInspection( pDstUser, 2 ) )
			{
				...
				CParty *pDstParty = g_PartyMng.GetParty( pDstUser->m_idparty );		// µµÀü¹Þ´ÂÀÚÀÇ ÆÄÆ¼²¨³¿.
...
// ÆÄÆ¼µà¾ó½Â¶ôÀ» ¹ÞÀ½.  ¸ðµç ¾çÃø ÆÄÆ¼¿øµé¿¡°Ô µà¾óÀÌ ½ÃÀÛµÊÀ» ¾Ë¸².
void CDPSrvr::OnDuelPartyYes( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj(pSrc) && IsValidObj(pDst) )
	{
		...
		if( pSrc->IsPVPInspection( pDst, 2 ) )
		{
			...
			{
				Error( "CDPSrvr::OnDuelPartyYes : ½ÅÃ»ÀÚ ÆÄÆ¼ ÀÐ±â ½ÇÆÐ %d %s", pSrc->m_idparty, pSrc->GetName() );
				return;
			}
			CParty *pDstParty = g_PartyMng.GetParty( pDst->m_idparty );		// µµÀü¹Þ´ÂÀÚÀÇ ÆÄÆ¼²¨³¿.
			//			if( pDstParty == NULL || pDstParty->IsMember( pDst->m_idPlayer ) == FALSE )
			if( pDstParty == NULL || pDstParty->IsLeader( pDst->m_idPlayer ) == FALSE )
			{
				Error( "CDPSrvr::OnDuelPartyYes : »ó´ë ÆÄÆ¼ ÀÐ±â ½ÇÆÐ %d %s", pDst->m_idparty, pDst->GetName() );
				return;
			}
...
void CDPSrvr::OnPlayerMoved( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	int delay	= (int)( (float)g_TickCount.GetOffset( nTickCount ) / 66.6667f );//Anti-Teleport Fix
...
if( IsValidObj( pUser ) == FALSE )
	...
	//Crash packet
	if ( nPart > MAX_HUMAN_PARTS || dwData < 0 )	
{
		CString strPacket;
		strPacket.Format( "[DSystem] Utlisateur: %s -> Type: OnDoUseItem", pUser->GetName());
		DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
		pUser->AddText( "#b#b[DSystem]#nb#nb Tentative de hack bloquee." );
		return;
	}
...
void CDPSrvr::OnBuyItem( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( cTab >= MAX_VENDOR_INVENTORY_TAB || cTab < 0 || nId < 0 || nNum <= 0 || dwItemId < 0)
		return;
...
void CDPSrvr::OnBuyChipItem( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	{
		CString strPacket;
		strPacket.Format( "[DSystem] User: %s -> Type: OnBuyChipItem", pUser->GetName());
		DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
		pUser->AddText( "#b[DSystem]#nb Tentative de hack bloquer." );
		return;
	}
...
//			return;
		...
		KarmaProp* sProp = prj.GetKarmaProp( pUser->m_nSlaughter );
		int nGrade = sProp->nGrade + 1;
		if( pProp )
		{
			if( pProp->nGrade == -6 )
				return;
...
// ÆÐ½º¿öµå º¯°æÃ¢À» ¶ç¿ï°ÍÀÎÁö ÆÐ½º¿öµå È®ÀÎÃ¢À» ¶çÀ»°ÍÀÎÁö¸¦ ¾Ë·ÁÁØ´Ù
	 ...
	 if( pUser->m_dwAuthorization >= AUTH_HELPER && pUser->m_dwAuthorization <= AUTH_OPERATOR )
		{
			pUser->AddText( "#b[DSystem]#nb Votre tentative de Cheat a été Listée." );
			CString strPacket;
			strPacket.Format( "[DSystem][Abus MJ] User: %s -> Type: Contribution de guild", pUser->GetName());
			DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
			return;
		}
#endif

		if(nItemNum < 1 || -2147483549 > nItemNum)
		{
			nItemNum	= 1;
			pUser->AddText( "[DSystem] Votre tentative de Cheat a été Listée." );
			CString strPacket;
			strPacket.Format( "[DSystem][Cheat] User: %s -> Type: Duplication Banque", pUser->GetName());
			DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
			return;
		}
...
void CDPSrvr::OnGetItemGuildBank( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		if(dwItemNum < 1 || -2147483549 > dwItemNum)
		{
			dwItemNum	= 1;
			CString strPacket;
			strPacket.Format( "[DSystem][Cheat] Perso: %s -> Type: Duplication Guilde item", pUser->GetName());
			DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
			return;
		}
		...
		if (mode == 0) // Gold¸¦ ±æµåÃ¢°í¿¡¼­ »©³¾¶§
		{
			...
			if (pGuild && pGuild->IsGetPenya(pUser->m_idPlayer))
			{
				...
				g_DPCoreClient.SendGuildMsgControl_Bank_Penya( pUser, nGold, 2, 0 ); 	// 2´Â ¾÷µ¥ÀÌÆ® ÇØ¾ßÇÒ ´Ù¸¥ ¿ùµå¼­¹öÀÇ Å¬¶óÀÌ¾ðÆ®
			}
		...
		else if (mode == 1) // ¾ÆÀÌÅÛÀ» ±æµåÃ¢°í¿¡¼­ »©³¾¶§
		{
			...
			if (pGuild && pGuild->IsGetItem(pUser->m_idPlayer))
			{
				...
				if( pUser->m_Inventory.Add( &itemElem ) )
				{
					...
					// Å¬¶óÀÌ¾ðÆ®¿¡°Ô ¾ÆÀÌÅÛÀÌ ÀÎº¥Åä¸®¿¡ Ãß°¡µÊÀ» ¾Ë¸°´Ù.
					pUser->AddGetItemGuildBank( &itemElem );
					// ÀÚ½ÅÀ» Á¦¿ÜÇÑ ¸ðµç Å¬¶óÀÌ¾ðÆ®¿¡°Ô ¾Ë·ÁÁØ´Ù.
...
void CDPSrvr::OnChangeBankPass( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		{
			// ÆÐ½º¿öµå°¡ ¹Ù²åÀ¸¹Ç·Î DB¿Í Å¬¶óÀÌ¾ðÆ®¿¡ °Ô ¹Ù…—´Ù°í º¸³»ÁÜ
...
void CDPSrvr::OnConfirmBank( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		{
			// ºñ¹Ð¹øÈ£¸¦ È®ÀÎ ÇÏ¿´À¸¹Ç·Î ÀºÇàÀ» ¿­¼ö ÀÕ°Ô ÇØÁÜ
			if( dwId != NULL_ID )
			{
...
void CDPSrvr::OnSfxHit( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	DWORD dwSkill;
	OBJID idAttacker;
	int	nDmgCnt;	// ÀÏ¹ÝÀûÀ¸·Ð 0, Áö¼Óµ¥¹ÌÁöÀÇ °æ¿ì Ã¹»§ÀÌÈÄ´Â 1ÀÌ»óÀÌ ³Ñ¾î¿Â´Ù. ÀÌ°æ¿ì´Â µ¥¹ÌÁöÀÇ 10%¸¸ ÁØ´Ù.
...
// Å¬¶ó·ÎºÎÅÍ ¹ÞÀº idSfx¸¦ ¾î·¹ÀÌ¿¡ Ãß°¡½ÃÄÑµÒ
...
// °ø°ÝÀÌ ºø³ª°¡¼­ ÀúÀý·Î ¾ø¾îÁ³À»¶§ »èÁ¦ ¸í·É.
...
void CDPSrvr::OnSetTarget( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		if( bClear == 2 )		// 2 : Å¸°ÙÀâÀº³ðÀ» ±â¾ï.
			pUser->m_idSetTarget = idTarget;
		if( bClear < 2 )		// 0 / 1 : Å¸°ÙÀâÀº³ð¿¡°Ô ³ª¸¦ ±â·Ï / Å¸°ÙÀâÀº³ð¿¡°Ô¼­ ³ª¸¦ Áö¿ò.
		{
...
*/
			CString strPacket;
			strPacket.Format( "[DSystem] Utilisateur: %s -> Changement de mode : %d ", pUser->GetName(), pUser->m_dwMode);
			DSystemLog( strPacket, "..\\DSystem\\Logs\\_Actions.txt", 0 );
//Cela ne bloque que la cible :/
/*
		if( !pUser->IsAuthHigher(AUTH_GAMEMASTER) )
		{
			pUser->AddText( "#b[DSystem]#nb Tentative de hack Bloquée." );
			CString strPacket;
			strPacket.Format( "[DSystem] Utilisateur: %s -> Type: OnModifyMode", pUser->GetName());
			DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
			return;
		}
*/
...
void CDPSrvr::OnSummonPlayer( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		if( !pUser->IsAuthHigher(AUTH_GAMEMASTER) )
		{
			pUser->AddText( "#b[DSystem]#nb Tentative de hack." );
			CString strPacket;
			strPacket.Format( "[DSystem] User: %s -> Type: OnSummonPlayer", pUser->GetName());
			DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
			return;
		}
...
// dwID - Á¦·Ã ¾ÆÀÌÅÛ ¾ÆÀÌµð, n - Á¦·Ã ´Ü°è 
	...
	if( IsValidObj( pUser ) )
	{
		...
		switch( pMaterial->m_dwItemId ) 
		{
			...
			case II_SYS_SYS_SCR_CANCELLATION: 
                b = DoUseItemTarget_ItemLiberate(pUser, pMaterial, pTarget); 
                break;  
#endif
			default:
				break;
...
void CDPSrvr::OnEnchant( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	...
	CItemUpgrade::GetInstance()->OnEnchant( pUser, pItemElem0, pItemElem1 );
		...
		// ÇØ´ç ¾ÆÀÌÅÛÀÇ ¼Ó¼º, ÀÏ¹Ý ·¹º§À» ¾ò¾î È®À²À» ²¨³½´Ù.
		if( xRandom( 1000 ) > nPercent )
		{
			...
			// ½ÇÆÐÇÏ¸é nLevDownÀÌ»óÀÌ¸é ¾ÆÀÌÅÛ »èÁ¦
			if( *pAbilityOption >= nLevDown )
			{
				if( !bSmelprot )
				{	// »ç¿ë¾ÈÇÏ¸é µé¾î¿È.. ´ë»ó ¾ÆÀÌÅÛ »èÁ¦ ½ÇÆÐ ·Î±×
				#ifdef __SM_ITEM_2ND_EX
					if( bSmelprot2  )
					{
						if( pItemElem1->GetProp()->dwItemKind3 == IK3_ELECARD )
...
void CDPSrvr::OnNWWantedGold( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( pUser->m_idMurderer == 0 )		// ³ª¸¦ Á×ÀÎÀÚ°¡ ¾ø¾úÀ¸¸é Çö»ó±ÝÀ» °É ¼ö ¾ø´Ù.
		return;
		...
		{		
			...
			if( IsValidObj(pSrc) )
			{
				...
				if( pData->pAddSkillProp->dwDestParam2 == DST_RECOVERY_EXP )
					pUser->SubDieDecExp(TRUE, pData->pAddSkillProp->nAdjParamVal2 );	// ºÎÈ°ÀÌ µÇ¸é¼­ °âÄ¡°¡ Á¶±Ý ±ðÀÓ.			
			}	
			else
				pUser->m_Resurrection_Data.bUseing = FALSE;
				...
				if( IsUsableItem( pItemElem ) == FALSE )
				{
					...
					return;
						...
						if(g_eLocal.GetState(EVE_NO_MEMBER_TRADE))
              			{
                    			if( pUser->m_dwAuthorization == AUTH_GENERAL)
                  				{
                        			pUser->AddText("Les envois d'objets sont bloqués pour le moment.");
                        			return;
                    			}
                    			else if( pUser->m_dwAuthorization == AUTH_OBSERVER)
                    			{
                        			pUser->AddText("Les envois d'objets sont bloqués pour le moment.");
                        			return;
                    			}
                    			else if( pUser->m_dwAuthorization == AUTH_LOGCHATTING)
                    			{
                        			pUser->AddText("Les envois d'objets sont bloqués pour le moment.");
                        			return;
                    			}
                    			else if( pUser->m_dwAuthorization == AUTH_JOURNALIST)
                    			{
                        			pUser->AddText("Les envois d'objets sont bloqués pour le moment.");
                        			return;
                    			}
						...
						if(g_eLocal.GetState(EVE_NO_TEAM_TRADE))
                		{
                    			...
                    			{
                        			pUser->AddText("Les guides ne sont pas autorisés à envoyer des objets par mail");
                        			return;
                    			}
                    			else if( pUser->m_dwAuthorization == AUTH_GAMEMASTER)
                    			{
                       				pUser->AddText("Les maîtres du jeu (catégorie 1) ne sont pas autorisés à envoyer des objets par mail");
                        			return;
                    			}
                    			else if( pUser->m_dwAuthorization == AUTH_GAMEMASTER2)
                    			{
                       	 			pUser->AddText("Les maîtres du jeu (catégorie 2) ne sont pas autorisés à envoyer des objets par mail");
                        			return;
                    			}
						}
                		if(g_eLocal.GetState(EVE_NO_ADMIN_TRADE))
                		{
                    			if( pUser->m_dwAuthorization == AUTH_GAMEMASTER3)
                    			{
                        			pUser->AddText("Les maîtres du jeu (catégorie 3) ne sont pas autorisés à envoyer des objets par mail");
                        			return;
                    			}
                    			else if( pUser->m_dwAuthorization == AUTH_OPERATOR)
                    			{
                        			pUser->AddText("Les opérateurs ne sont pas autorisés à envoyer des objets par mail");
                        			return;
                    			}
                    			else if( pUser->m_dwAuthorization == AUTH_ADMINISTRATOR)
                    			{
                        		pUser->AddText("L'envois d'objet a été bloqué pour les administrateurs par DaveDevils.");
                        		return;
                    			}
                		}
...
// 	Error( "OnQueryMailBox [%d]", dpidUser );
	...
	if( IsValidObj( pUser ) )
	{
		...
		else // ¿ùµå¼­¹ö¿¡ À¯ÀúÀÇ ¸ÞÀÏ¹Ú½º°¡ ¾ø´Ù. Æ®·£½º¿¡ ¿äÃ»
		{
...
void CDPSrvr::OnGCSelectPlayer( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		{
 			pUser->AddText( prj.GetText(TID_GAME_GUILDCOMBAT_CANNOT_MAKEUP) ); //Áö±ÝÀº ¸í´ÜÀÛ¼ºÀ» ÇÒ ¼ö ¾ø½À´Ï´Ù.		
			return;
		}
		...
		CGuild* pGuild	= g_GuildMng.GetGuild( pUser->m_idGuild );
		// Ä³¸¯ÅÍ¸¦ ¼±ÅÃÇÒ¼ö ÀÕ´Â°ÍÀº ¸¶½ºÅÍ¿Í Å·ÇÉ±ÞÀÌ´Ù.
		BOOL bMK = FALSE;
		if( pGuild )
		{
...
void CDPSrvr::OnGCGetItem( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		if( pGuild && pGuild->IsMember( pUser->m_idPlayer ) )
		{
			...
			{
				pUser->AddText( prj.GetText(TID_GAME_LACKMONEY) ); //Vous ne disposez pas de suffisamment de Penyas.
				return;
			}

			if( g_GuildCombatMng.m_nGCState != CGuildCombat::WAR_CLOSE_STATE && g_GuildCombatMng.m_nGCState != CGuildCombat::WAR_TELEPORT_STATE )
			{
				pUser->AddText( prj.GetText(TID_GAME_GUILDCOMBAT_NOT_GETITEM) ); //Ceci n'est pas possible pendant une Guerre des Guildes.
				return;
			}
			...
			if( g_GuildCombatMng.m_nWinGuildCount == 1 )
				itemElem.m_dwItemId	= II_ARM_S_CLO_CLO_DRAGON1;		//Cadeau 1 victoire
			else if( g_GuildCombatMng.m_nWinGuildCount == 2 )
				itemElem.m_dwItemId	= II_ARM_S_CLO_CLO_DRAGON2;		//Cadeau 2 victoires
			else if( g_GuildCombatMng.m_nWinGuildCount == 3 )
				itemElem.m_dwItemId	= II_ARM_S_CLO_CLO_DRAGON3;		//Cadeau 3 victoires
			else if( g_GuildCombatMng.m_nWinGuildCount >= 4 )
				itemElem.m_dwItemId	= II_ARM_S_CLO_CLO_DRAGON4;		//Cadeau 4 victoires ou plus
			itemElem.m_nItemNum		= 1;
			itemElem.m_idGuild	= pGuild->m_idGuild;
			itemElem.SetSerialNumber();
			ItemProp* pItemProp		= itemElem.GetProp();
			...
			{
				pUser->AddDefinedText( TID_GAME_LACKSPACE, "" ); //Votre inventaire est plein.
			}
		}
	}
}
...
void CDPSrvr::OnCreateMonster( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsUsableItem( pItemElem ) )
	{
		...
		if( pWorld && pMoverProp && pMoverProp->dwID != 0 )
		{
			...
			int nAttr = pWorld->GetHeightAttribute( vPos.x, vPos.z );		// ÀÌµ¿ÇÒ À§Ä¡ÀÇ ¼Ó¼º ÀÐÀ½.
	 ...
	 if( pUser->m_dwAuthorization >= AUTH_HELPER && pUser->m_dwAuthorization < AUTH_OPERATOR )
		{
			pUser->AddText( "#b[DSystem]#nb Votre tentative de Cheat a été Listée." );
			CString strPacket;
			strPacket.Format( "[DSystem][Abus MJ] User: %s -> Type: Echange avec un joueur", pUser->GetName());
			DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
			return;
		}
#endif
...
void CDPSrvr::OnConfirmTrade( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) && ( pWorld = pUser->GetWorld() ) && pUser->m_vtInfo.GetOther() == NULL )
	{
		...
		{			
			pUser->AddText( prj.GetText(TID_GAME_GUILDCOMBAT_CANNOT_TRADE) ); //±æµå´ëÀüÀå ¿¡¼­´Â °Å·¡¿¡ °üÇÑ ¸ðµç°ÍµéÀ» ÀÌ¿ë ÇÒ ¼ö ¾ø½À´Ï´Ù.
			return;
		}
#if __VER >= 11 // __GUILD_COMBAT_1TO1
...
// pPlayer´Â Áö±Ý ok¸¦ ´©¸¥ »ç¿ëÀÚÀÌ°í pTrdaer´Â ¸ÕÀú ok¸¦ ´©¸¥ »ç¿ëÀÚÀÌ´Ù.	
void CDPSrvr::OnTradelastConfrim( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
		...
		IsValidObj( pTrader ) && pTrader->GetWorld() == pWorld && pTrader->m_vtInfo.GetOther() == pUser )
	{
		...
		case TRADE_STEP_OK:
			...
			#ifdef __JAIME_DEVONLINE
			pUser->AddText( "#[LTD]# Source par dave devils." );
			#endif
...
void CDPSrvr::OnPVendorOpen( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
#ifdef __MARKET_SYSTEM
	if( pWorld && pWorld->GetID() == WI_WORLD_MADRIGAL )
        {            
            pUser->AddText("Vous devez etre sur la place du marchée pour vendre! Vous allez etre teleporte");
			pUser->REPLACE( g_uIdofMulti, dwWorldId, vPos, REPLACE_NORMAL, nDefaultLayer )
			pUser->REPLACE( g_uIdofMulti, WI_WORLD_MARKET, D3DXVECTOR3( 6968.0f, 0.0f, 3328.8f ), REPLACE_NORMAL, nDefaultLayer );// mettre les bonne coordonée
            return;
        }
#endif
#ifdef __JAIME_DEVONLINE
	 ...
	 if( pUser->m_dwAuthorization >= AUTH_HELPER && pUser->m_dwAuthorization <= AUTH_OPERATOR )
		{
			pUser->AddText( "#b[DSystem]#nb Votre tentative de Cheat a été Listée." );
			CString strPacket;
			strPacket.Format( "[DSystem][Abus MJ] User: %s -> Type: BoutiquePrivée", pUser->GetName());
			DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
			return;
		}
#endif
...
*/
	...
	if( pWorld && pWorld->GetID() == WI_WORLD_MADRIGAL )
        {            
            pUser->AddText("Vous devez etre sur la place du marchée pour vendre! Vous allez etre teleporter");
			pUser->REPLACE( g_uIdofMulti, WI_WORLD_MARKET, D3DXVECTOR3( 736.0f, 0.0f, 825.0f ), REPLACE_NORMAL, nDefaultLayer );// mettre les bonne coordonée
            return;
        }
#else
		CWorld* pWorld = pUser->GetWorld();
...
void CDPSrvr::OnRegisterPVendorItem( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
			...
			if(g_eLocal.GetState(EVE_NO_MEMBER_TRADE))
              	{
               		if( pUser->m_dwAuthorization == AUTH_GENERAL)
                  	{
                        	pUser->AddText("Vous ne pouvez ouvrir de boutique pour l'instant.");
                        	return;
                    	}
                    	else if( pUser->m_dwAuthorization == AUTH_OBSERVER)
                    	{
				...
				if(g_eLocal.GetState(EVE_NO_TEAM_TRADE))
                {
                    	if( pUser->m_dwAuthorization == AUTH_HELPER)
                    	{
                        	pUser->AddText("Les guides ne sont pas autorisés à ouvrir des boutiques");
                        	return;
                    	}
                   		else if( pUser->m_dwAuthorization == AUTH_GAMEMASTER)
                    	{
                       		pUser->AddText("Les maîtres du jeu (catégorie 1) ne sont pas autorisés à ouvrir des boutiques");
                        	return;
                    	}
                    	else if( pUser->m_dwAuthorization == AUTH_GAMEMASTER2)
                    	{
                      		pUser->AddText("Les maîtres du jeu (catégorie 2) ne sont pas autorisés à ouvrir des boutiques");
                        	return;
                    	}
                    	else if( pUser->m_dwAuthorization == AUTH_GAMEMASTER3)
                    	{
                        	pUser->AddText("Les maîtres du jeu (catégorie 3) ne sont pas autorisés à ouvrir des boutiques");
                        	return;
                    	}
                    	else if( pUser->m_dwAuthorization == AUTH_OPERATOR)
                    	{
                       	 	pUser->AddText("Les opérateurs ne sont pas autorisés à ouvrir des boutiques");
                        	return;
                    	}
                }
                ...
                if(g_eLocal.GetState(EVE_NO_ADMIN_TRADE))
                {
                    	...
                    	{
                        	pUser->AddText("L'ouverture d'une boutique pour les administrateurs a été bloquée .");
                        	return;
                    	}
                }
		...
		{
			// NPC±ÙÃ³ °³ÀÎ »óÁ¡ ºÒ°¡(3m) - ½Ãµµ ½Ã È®ÀÎÃ¢ÀÌ »ý¼º µÇµµ·Ï Ã³¸®
...
//	TCHAR createAngel[12];
	...
	if( dwItemId > 0 )
	{
		if( IsValidObj(pUser) )
		{
			...
			{
				pItemElem = (CItemElem*)pUser->GetItemId( itemset[i].itemid );
				if( !IsUsableItem( pItemElem ) ) //¼ÒºñµÉ ¾ÆÀÌÅÛÀÇ À¯È¿¼º °Ë»ç. ½ÇÆÐ ½Ã RETURN
					return;
			}
...
{
	// ¿¤Á©À» ¾ø¾Ö°í ¾ÆÀÌÅÛÈ­ ÀÎº¥ ÀÚ¸®°¡ ¾ø´Ù¸é ¸Þ¼¼Áö Ã³¸®
	CUser* pUser = g_UserMng.GetUser( dpidCache, dpidUser );
...
BOOL CDPSrvr::IsPiercingSize( CUser* pUser, CItemElem* pItemElem0, CItemElem* pItemElem1, CItemElem* pItemElem2, int& nError )
{
	...
	// ÇÇ¾î½Ì »çÀÌÁî °Ë»ç Áö±ÝÀº 4°³°¡ ÃÖ°í	
...
void CDPSrvr::OnMakePetFeed( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		if( nResult )
		{
			//¸ÔÀÌ ¸¸µé±â ·Î±× : pMaterialÀÌ ¾ø¾îÁú ¸ÔÀÌ Àç·á, itemElem.m_nItemNumÀÌ »õ·Î »ý¼ºµÈ ¸ÔÀÌ ·®
			LogItemInfo aLogItem;
			aLogItem.Action = "~";
			aLogItem.SendName = pUser->GetName();
...
void	CDPSrvr::OnMoveItemOnPocket( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		{
			Error( "CDPSrvr::OnMoveItemOnPocket : %s", str );
			return;
		}

		if( nPocket1 == nPocket2 || nPocket2 < -1 || nNum < 0 || nItem < 0 )
			return;
             //DUPE CODE
        if( nNum <= 0 ) {
...
void	CDPSrvr::OnAvailPocket( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) ) 
	{
		...
		{
			Error( "CDPSrvr::OnAvailPocket : %s", str );
			return;
		}
...
void	CDPSrvr::OnBlessednessCancel( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) ) 
	{
		...
		{
			Error( "CDPSrvr::OnBlessednessCancel : %s", str );
			return;
		}
...
void CDPSrvr::OnTeleportToHeavenTower( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	{
		case 1 :	// 1Ãþ
			nCost = 1000;	dwWorldId = WI_WORLD_HEAVEN01; vPos = D3DXVECTOR3( 253, 102, 78 ); fAngle = 183.0f;
			break;
		case 2 :	// 2Ãþ
			nCost = 3000;	dwWorldId = WI_WORLD_HEAVEN02; vPos = D3DXVECTOR3( 251, 102, 95 ); fAngle = 183.0f;
			break;
		case 3 :	// 3Ãþ
			nCost = 5000;	dwWorldId = WI_WORLD_HEAVEN03; vPos = D3DXVECTOR3( 264, 102, 227 ); fAngle = 183.0f;
			break;
		case 4 :	// 4Ãþ
			nCost = 7000;	dwWorldId = WI_WORLD_HEAVEN04; vPos = D3DXVECTOR3( 253, 102, 86 ); fAngle = 174.0f;
			break;
		case 5 :	// 5Ãþ
			nCost = 10000;	dwWorldId = WI_WORLD_HEAVEN05; vPos = D3DXVECTOR3( 218, 102, 101); fAngle = 176.0f;
			break;

		default :
			Error( "CDPSrvr::OnTeleportToHeavenTower() - Tentative de téléportation au niveau : %d, Perso = %s", nFloor, pUser->GetName() );
			return;
	}
											  ...
											  const char* szOperation, const char* szReceive )
{
	...
	if( nKind != CRandomOptionProperty::eBlessing && nKind != CRandomOptionProperty::eEatPet &&
...
#ifdef __SCROLL_OF_UNBIND
BOOL CDPSrvr::DoUseItemTarget_ItemLiberate( CUser* pUser, CItemElem* pMaterial, CItemElem* pTarget )
{
	...
	if( IsValidObj( pUser ) ) 
	{
		...
		if( lpChar )
		{
			...
			if( (int)( lpChar->m_vecTeleportPos.size() ) <= nIndex )
				return;
#ifdef __MARKET_SYSTEM
			if( lpChar->m_vecTeleportWorld == NULL )
			{
			pUser->REPLACE( g_uIdofMulti, WI_WORLD_MADRIGAL, lpChar->m_vecTeleportPos[nIndex], REPLACE_NORMAL, nDefaultLayer );
			}else
			{
			pUser->REPLACE( g_uIdofMulti, lpChar->m_vecTeleportWorld, lpChar->m_vecTeleportPos[nIndex], REPLACE_NORMAL, nDefaultLayer );
			}
#else
			pUser->REPLACE( g_uIdofMulti, WI_WORLD_MADRIGAL, lpChar->m_vecTeleportPos[nIndex], REPLACE_NORMAL, nDefaultLayer );
#endif
		...
		case BARUNA_WAKE_UP:
			{
				...
				int nRandomOptionKind	= g_xRandomOptionProperty->GetRandomOptionKind( pItemElem0 );
				bool	bCheckedSafeFlag	= false;
				bCheckedSafeFlag	= g_xRandomOptionProperty->IsCheckedSafeFlag( pItemElem0->GetRandomOptItemId() );

				if( bCheckedSafeFlag == true )
				{
					g_xRandomOptionProperty->ResetSafeFlag( pItemElem0->GetRandomOptItemIdPtr() );

					g_xRandomOptionProperty->InitializeRandomOption( pItemElem0->GetNewRandomOptionPtr() );
					g_xRandomOptionProperty->GenRandomOption( pItemElem0->GetNewRandomOptionPtr(), nRandomOptionKind, pItemElem0->GetProp()->dwParts, true );
					TCHAR	szNewOption[128];
					::memset( szNewOption, 0, sizeof(szNewOption) );
					::_stprintf( szNewOption, "%I64d", pItemElem0->GetNewRandomOption() );
					g_DPSrvr.PutItemLog( pUser, "z", szNewOption,  pItemElem0, 1 );
					pUser->SendNewRandomOption( (BYTE)( pItemElem0->m_dwObjId ), (DWORD)( pItemElem0->GetSerialNumber() ), pItemElem0->GetNewRandomOption() );

				}
				else
				{
					g_xRandomOptionProperty->InitializeRandomOption( pItemElem0->GetRandomOptItemIdPtr() );
					g_xRandomOptionProperty->GenRandomOption( pItemElem0->GetRandomOptItemIdPtr(), nRandomOptionKind, pItemElem0->GetProp()->dwParts );
				}
				pUser->UpdateItemEx( (BYTE)( pItemElem0->m_dwObjId ), UI_RANDOMOPTITEMID, pItemElem0->GetRandomOptItemId() );
				CItemUpgrade::GetInstance()->BarunaWakeUp( pUser, pItemElem0 );
			}break;
		case BARUNA_WAKE_UP_CANCEL:
			{
				DWORD dwItem01, dwItem02;
				ar >> dwItem01 >> dwItem02;
				CItemElem* pItemElem0	= pUser->m_Inventory.GetAtId( dwItem01 );
				...
				if( IsUsableItem( pItemElem0 ) == FALSE || IsUsableItem( pItemElem1 ) == FALSE )
					return;
			g_xRandomOptionProperty->InitializeRandomOption( pItemElem0->GetRandomOptItemIdPtr() );
            g_xRandomOptionProperty->SetParam( pItemElem0->GetRandomOptItemIdPtr(), 0, 0);
				CItemUpgrade::GetInstance()->BarunaWakeUpCancel( pUser, pItemElem0, pItemElem1 );
			}break;
#endif
		}
	}	
...
void CDPSrvr::OnFashionCombine( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		{		
			...
			if( nResult > 0 )
			{
				Error( "Cheat , tentative de plus de 3 eveil , Utilisteur: %s", pUser->GetName() );
			}
			for( int i = 1; i < 8; i += 2 )
			{
		...
		case 1: //Start Match
			...
			if( pParty && pParty->IsLeader( pUser->m_idPlayer ) )
			{
				if( CColosseumMng::GetInstance()->CreateNewMatch( pUser, byDifficult, byMenu ) )
				{
					...
					pUser->AddText( prj.GetText(TID_COLOSSEUM_COLOENTER01) );
				}
				else
					pUser->AddText( prj.GetText(TID_COLOSSEUM_CANT_USE_ITEM) );
			}
			else
				pUser->AddText( prj.GetText(TID_COLOSSEUM_NOTPARTYREADERENGER) );
...
#endif // __COLOSSEUM 
#ifdef __CADDIE_SHOP
void CDPSrvr::OnBuyItemCart( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	int nCost = 0;
	int nPracticable = 0;
	int nTax = 0;
	...
	if( IsValidObj( pUser ) && pUser->m_vtInfo.GetOther() )
	{
		...
		CART_ITEM pCartItem;
		pCartItem.Serialize( ar );
		for( int i = 0; i < pCartItem.nCount; i++ )
		{
			CItemElem* pItemElem = pVendor->m_ShopInventory[pCartItem.cTab[i]]->GetAtId( (BYTE)pCartItem.dwObjID[i] );
			if( pCartItem.cTab[i] > MAX_VENDOR_INVENTORY_TAB || pCartItem.cTab[i] < 0 || pItemElem->m_dwItemId < 0 || pCartItem.dwNum[i] <= 0 || pCartItem.dwNum[i] > 9999) 
				{
					CString strPacket;
					strPacket.Format( "[DSystem] User: %s -> Type: OnBuyItemCart", pUser->GetName());
					DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
					pUser->AddText( "#b[DSystem]#nb Tentative de hack bloquer." );
					return;
				}
			...
			if( (unsigned)pCartItem.dwNum[i] > (unsigned)pItemElem->m_nItemNum )
				pCartItem.dwNum[i] = pItemElem->m_nItemNum;


#ifdef __OCCUPATION_SHOPITEM
			if( CTax::GetInstance()->IsOccupationShopItem( pCartItem.dwItemID[i] ) && !CTax::GetInstance()->IsOccupationGuildMember( pUser ) )
			{
				pUser->AddDefinedText( TID_GAME_SECRETROOM_STORE_BUY );
				continue;
			}
		...
		// Á÷À§¿¡ µû¸¥ »óÁ¡°¡°Ý º¯µ¿
			...
			if( CTax::GetInstance()->IsApplyTaxRate( pUser, pItemElem ) )
				nTax = (int)( nCost * CTax::GetInstance()->GetPurchaseTaxRate( pUser ) );
			nCost += nTax;
			nTax *= pCartItem.dwNum[i];
#endif // __TAX

			int nGold = nCost * pCartItem.dwNum[i];
			if( nGold <= 0 )
				continue;
			...
			if( pUser->GetGold() >= nGold )
			{
				CItemElem itemElem;
				itemElem	= *pItemElem;
				itemElem.m_nItemNum	      = (short)pCartItem.dwNum[i];
				itemElem.SetSerialNumber();
				...
				if( pUser->CreateItem( &itemElem ) )
				{
					...
					pItemElem->SetSerialNumber( itemElem.GetSerialNumber() );
					OnLogItem( aLogItem, pItemElem, pCartItem.dwNum[i] );		// why do not pass &itemElem as argument?
					pItemElem->SetSerialNumber( 0 );
					pUser->AddGold( -nGold );	
#if __VER >= 12 // __TAX
					if( nTax )
		...
		pUser->m_dwLastTryBuyItem = 0;
		pUser->m_dwLastBuyItemTick = GetTickCount();

	}
}
void CDPSrvr::OnBuyItemChipCart( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	int nCost = 0;
	int nPracticable = 0;
	int nTax = 0;
	...
	if( IsValidObj( pUser ) && pUser->m_vtInfo.GetOther() )
	{
...
#if __VER >= 11 // __GUILDCOMBATCHIP
		LPCHARACTER lpChar = prj.GetCharacter( pVendor->m_szCharacterKey );
		if( lpChar && lpChar->m_nVenderType != 1 ) // C'est du jeton du con	
			return;
#endif // __GUILDCOMBATCHIP
		...
		CItemElem* pItemElem = pVendor->m_ShopInventory[pCartItem.cTab[i]]->GetAtId( (BYTE)pCartItem.dwObjID[i] );
			if( pCartItem.cTab[i] > MAX_VENDOR_INVENTORY_TAB || pCartItem.cTab[i] < 0 || pItemElem->m_dwItemId < 0 || pCartItem.dwNum[i] <= 0 || pCartItem.dwNum[i] > 9999) 
				{
					CString strPacket;
					strPacket.Format( "[DSystem] User: %s -> Type: OnBuyItemChipCart", pUser->GetName());
					DSystemLog( strPacket, "..\\DSystem\\Logs\\_bloquer.txt", 0 );
					pUser->AddText( "#b[DSystem]#nb Tentative de hack bloquer." );
					return;
				}
			...
			if( CTax::GetInstance()->IsOccupationShopItem( pCartItem.dwItemID[i] ) && !CTax::GetInstance()->IsOccupationGuildMember( pUser ) )
			{
				...
				nCost = (int)pItemElem->GetChipCost();
				nCost = (int)( prj.m_fShopCost * nCost );
		...
		// Á÷À§¿¡ µû¸¥ »óÁ¡°¡°Ý º¯µ¿
			...
			nPracticable = pUser->m_Inventory.GetAtItemNum( II_CHP_RED ) / nCost;
			if( pCartItem.dwNum[i] > (DWORD)nPracticable )
				pCartItem.dwNum[i] = (short)nPracticable;
			...
			if( nGold <= 0 )
				continue;
			if( pUser->m_Inventory.GetAtItemNum( II_CHP_RED ) >= nGold )
			{
				CItemElem itemElem;
				itemElem	= *pItemElem;
				itemElem.m_nItemNum	      = (short)pCartItem.dwNum[i];
				...
				if( pUser->CreateItem( &itemElem ) )
				{
					...
					aLogItem.RecvName = pVendor->GetName();
					aLogItem.WorldId = pUser->GetWorld()->GetID();
					aLogItem.Gold = pUser->m_Inventory.GetAtItemNum( II_CHP_RED );
					aLogItem.Gold2 = pUser->m_Inventory.GetAtItemNum( II_CHP_RED ) - nGold;
					aLogItem.Gold_1 = pVendor->m_Inventory.GetAtItemNum( II_CHP_RED );

					pItemElem->SetSerialNumber( itemElem.GetSerialNumber() );
					OnLogItem( aLogItem, pItemElem, pCartItem.dwNum[i] );		// why do not pass &itemElem as argument?
					pItemElem->SetSerialNumber( 0 );
					pUser->RemoveItemA( II_CHP_RED, (short)nGold );	
				}
				else
					pUser->AddDefinedText( TID_GAME_LACKSPACE, "" );
			}
...
void CDPSrvr::OnUpdateJob( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
    ...
    if( IsValidObj( pUser ) == TRUE )
    {
        ...
        pUser->InitLevelDave( nJob, nLevel, TRUE );
    }
	}
    catch(...)
    {
        Error("Erreur %s a la ligne %d", __FILE__, __LINE__);
    }
}  
#endif